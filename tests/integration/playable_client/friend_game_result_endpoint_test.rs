...
use server_crate::core::session::{
    GameSessionPlugin, LobbyState, PlayerConnectionMap, RoomSessions, ServerRngFactory,
    ServerRngInitError, SessionConfig,
};
...
use shared::protocol::{
    ...
    PlacedCardSubmit, PlayTarget, ReliableChannel, ResolutionEvent,
...
fn real_friend_game_route_reaches_game_over_result_endpoint() {
    ...
    for _ in 0..ROOM_FLOW_MAX_FRAMES {
        ...
        if flags.game_over_result_observed() {
            break;
        }
    ...
    assert!(
        flags
            ...
            && flags
                ...
                .load(Ordering::SeqCst),
        "route should extend beyond the Sprint 7 next-loop DraftShop endpoint: {}",
        flags.report()
    );
    assert!(
    ...
    assert!(
        flags
            ...
            && flags
                ...
                .load(Ordering::SeqCst),
        "S2CResolutionEvent should be observed before S2CPhaseChanged(GameOver): {}",
        flags.report()
    );
    assert!(
        flags
            ...
            && flags
                ...
                .load(Ordering::SeqCst),
        "result route should include objective destruction in the real resolution event: {}",
        flags.report()
    );
    assert_eq!(
        flags.game_over_loser.load(Ordering::SeqCst),
        NO_GAME_OVER_LOSER,
        "mutual second real-objective destruction should produce a draw: {}",
        flags.report()
    );
    assert!(
        flags.game_over_reason_draw.load(Ordering::SeqCst),
        "game-over reason should be Draw for simultaneous endpoint destruction: {}",
        flags.report()
    );
    assert!(
        flags.server_host_real_destroyed.load(Ordering::SeqCst) >= 2
            && flags.server_joiner_real_destroyed.load(Ordering::SeqCst) >= 2,
        "server ObjectiveCounters should reach the GAME_OVER threshold for both players: {}",
        flags.report()
    );
    assert!(
        ...
        "GSS should tear down session resources after game-over: {}",
        flags.report()
    );
}
...
impl RoomSessionFlags {
    ...
    fn report(&self) -> String {
        ...
        format!(
            "{route}, host_objective_identities={}, joiner_objective_identities={}, host_real_lane_mask={:#07b}, joiner_real_lane_mask={:#07b}, host_result_ready={}, joiner_result_ready={}, host_result_submit={}, joiner_result_submit={}, host_objective_destroyed_event={}, joiner_objective_destroyed_event={}, host_game_over={}, joiner_game_over={}, host_game_over_phase={}, joiner_game_over_phase={}, host_game_over_phase_after_resolution={}, joiner_game_over_phase_after_resolution={}, game_over_loser={}, game_over_round={}, game_over_reason_draw={}, server_round_game_over={}, server_lobby_game_over={}, server_session_config_removed={}, server_rng_removed={}, server_host_real_destroyed={}, server_joiner_real_destroyed={}",
            ...
            self.host_sent_result_endpoint_draft_shop_ready
                .load(Ordering::SeqCst),
            self.joiner_sent_result_endpoint_draft_shop_ready
                .load(Ordering::SeqCst),
            self.host_sent_result_endpoint_placement_submit
                .load(Ordering::SeqCst),
            self.joiner_sent_result_endpoint_placement_submit
                .load(Ordering::SeqCst),
            self.host_received_objective_destroyed_resolution_event
                .load(Ordering::SeqCst),
            self.joiner_received_objective_destroyed_resolution_event
                .load(Ordering::SeqCst),
            self.host_received_game_over.load(Ordering::SeqCst),
            self.joiner_received_game_over.load(Ordering::SeqCst),
            self.host_received_game_over_phase.load(Ordering::SeqCst),
            self.joiner_received_game_over_phase.load(Ordering::SeqCst),
            self.host_game_over_phase_after_resolution
                .load(Ordering::SeqCst),
            self.joiner_game_over_phase_after_resolution
                .load(Ordering::SeqCst),
...
fn deterministic_server_rng() -> Result<ServerRng, ServerRngInitError> {
    Ok(ServerRng::from_seed(0x504C_4159_A004))
}
