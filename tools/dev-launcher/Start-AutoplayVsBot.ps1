...
if (-not (Test-Path (Join-Path $RepoRoot 'Cargo.toml'))) {
    if ($DryRun) {
        Write-Host -ForegroundColor Yellow "[DRY RUN] would block in a real run: no Cargo.toml at $RepoRoot -- does not look like the CCGS workspace."
    } else {
...
# ---- 10. Fast-lane evidence paths + window-size visibility (PROMPT 1837 / refreshed PROMPT 1876) ----
