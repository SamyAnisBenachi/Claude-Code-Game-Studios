# PROMPT 1648 â€” AUTOPLAY-COMPOSITE-HARNESS-STATIC-HARDENING
...
### BUG-1 â€” Exit-code 11 comment listed only one of three trigger conditions
...
### BUG-2 â€” DryRun smoke preview omitted `-Python` and `-ClientStartupSecs`
...
### BUG-3 â€” DryRun composite outcome printed the live-run success message
**Location**: lines 368â€“370 (original)
**Severity**: Low (confusing â€” "Composite run COMPLETE" appeared even when nothing ran)
...
### NOTE-1 â€” Cosmetic: two section headers for smoke in DryRun
...
### NOTE-2 â€” Outcome switch maps exit codes 10/11/12 from smoke child
**Location**: lines 335â€“342
...
1. Header comment exit-11 line â†’ expanded to list all 3 conditions.
2. `Show-Help` exit-11 line â†’ expanded to list all 3 conditions.
3. DryRun smoke message â†’ added `-Python $Python -ClientStartupSecs $ClientStartupSecs`.
4. Section-9 outcome branch â†’ added `if ($DryRun)` guard with distinct DryRun message.
...
```
-Help  : PASS â€” exit-11 description now shows all 3 conditions.
-DryRun: PASS â€” smoke preview now shows full 7-arg command.
           PASS â€” outcome line reads "[DRY RUN] Simulated outcome: COMPLETE".
           PASS â€” no misleading "Composite run COMPLETE" in dry-run.
git diff --check: PASS
```
