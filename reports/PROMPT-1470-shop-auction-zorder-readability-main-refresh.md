...
- Refreshed PROMPT-1462 shop/auction readability repair onto current `origin/main` at `7e308a83c2f326bc8eb0f87804a13e813221f313`.
...
- Branch push was attempted after commit, but the required escalation was rejected by policy because exporting to the GitHub remote was not approved in this session. The local branch is committed and ready for orchestrator main-land.

## Verification

- `git diff --check origin/main...HEAD`: passed after rebase.
- `cargo test -p client --test shop_auction_ui_responsive_layout_repair_test --test shop_auction_ui_shop_panel_test --test shop_auction_ui_auction_featured_card_layout_test --test shop_auction_ui_auction_free_gold_counters_layout_test`: passed after rebase.
