...
**Source tree:** `origin/main@79031021` (PROMPT 1931 — latest main)
...
The 1891 and 1907 reports were also confirmed on `origin/main` before this
report was authored — no backfill is required.

This report is authored on a fresh branch from `origin/main@79031021` and
updates all integration branch and AC item status statements to reflect the
current state.
...
| **Verified on main** | `client/src/autoplay.rs`: `enforce_autoplay_window_size_system` at startup; `CCGS_WINDOW_WIDTH`/`CCGS_WINDOW_HEIGHT` env-var constants confirmed at lines 62–63, 140 |
...
| **Merge action** | Rebase onto `79031021`, then `git merge --ff-only` |
...
| **Registry** | `placement_reject_probe` entry absent from main `recipes/__init__.py` |

**Path to DONE:** Rebase `integrate/autoplay-placement-reject-recipe-1881` onto
`79031021`, FF merge, then run the recipe against a live game to produce pass
evidence.
...
| Worktree used (not root checkout) | PASS — `D:/tmp/wt-1935-bot-autoplay-readiness` |
