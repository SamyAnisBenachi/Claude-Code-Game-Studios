...
| **Autoplay vs Bot QA** (new) | — | col 5, span 3 |
...
- `Start-AutoplayVsBot.ps1` — untouched (owned by PROMPT 1648)
- All game client/server/shared Rust — untouched
- Sprint/session-state/story files — untouched
- Rebuild and Start Two-Client logic — unchanged, only layout spans adjusted

## Validation

- `git diff --check` — clean (no trailing whitespace)
- `cargo test` in `tools/dev-launcher-app` — 87 passed, 0 failed
...
2. Click "Autoplay vs Bot QA" — status should change to RUNNING (blue)
3. If run non-interactively, script exits 10 → badge should turn **yellow** and read `BLOCKED - Autoplay vs Bot QA exited 10 (...)`
4. If script is missing, script exits 11 → badge turns **yellow** (`BLOCKED`)
