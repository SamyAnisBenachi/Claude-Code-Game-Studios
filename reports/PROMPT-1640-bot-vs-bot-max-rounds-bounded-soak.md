...
| `client/src/presentation/result_screen.rs` | Refresh fix — handles `GameOverReason::MaxRoundsReached` with soak-complete result copy |
| `client/src/state/idempotency.rs` | Refresh fix — gives `MaxRoundsReached` a stable dedupe reason index |
...
During orchestrator refresh onto post-PROMPT-1643 `origin/main`, static review
found client-side exhaustive matches on `GameOverReason`. The refresh commit
therefore adds the minimal client handling required for `MaxRoundsReached`:
dedupe index `4` and result-screen copy `SOAK COMPLETE`.
