...
| **Verification Required** | (1) Confirm `PickingBehavior` only inserted behind `#[cfg(feature = "ui_picking")]` — inserting without the feature panics at runtime (unregistered component). (2) **bevy_tweening 0.18**: Confirm `Lens<T>` trait method signature — method may be named `lerp()` or `lens()` depending on crate version; verify against bevy_tweening 0.18 source before first Card Animations story. (3) **bevy_tweening 0.18**: Confirm `Animator<T>::set_tweenable()` method exists by that name — board-rendering.md specifies this API but it is unverified against the actual 0.18 release; run `cargo check` against a stub implementation before committing. (4) Confirm `TextureAtlas` in 0.18 is a component field (`texture_atlas: Some(TextureAtlas { layout: Handle<TextureAtlasLayout>, index })`) NOT a `Handle<TextureAtlas>` asset. (5) Confirm `Color::srgba(...)` is the correct constructor (not `Color::rgba`, renamed 0.15). (6) Confirm world-space sprites and bevy_ui panels cannot overlap: bevy_ui renders above all world-space content in a separate pass; achieving board content above a UI panel requires a custom render layer setup — verify this boundary holds for the hand drag sprite edge case. |
