...
**ADR Decision Summary**: `leader_snapshot_system` runs **post-SS1** (after all SS1 APPEARANCE effects resolve, before SS2 begins) — NOT on `ResolutionPhaseEntered`. It scans all LEADER units not silenced this round and writes `leader_bonus_atk`/`leader_bonus_hp` onto eligible family members' `UnitKeywordState`. LEADER bonus is snapshotted once per RESOLUTION — persists even if LEADER dies in SS4. Fields cleared at the start of the next round's snapshot run (overwritten to 0). **⚠️ ADR-018 needs a formal update** to change the trigger from `ResolutionPhaseEntered` to a post-SS1 signal.
...
**Engine Notes**: `leader_snapshot_system` uses `MessageWriter<KeywordTriggered>` — `app.add_message::<KeywordTriggered>()` must be registered in `KeywordPlugin` (Story 001). System runs on a **post-SS1 signal** (to be defined in ADR-018 update — e.g. `SS1Complete` or inline callback from `resolve_combat` after SS1 drains), NOT on `ResolutionPhaseEntered` — coordinate with RSM event bus (ADR-010).

**Control Manifest Rules (Feature layer)**:
- Required: `leader_snapshot_system` fires post-SS1 (after SS1 fully drains), NOT on `ResolutionPhaseEntered` — see ADR-018 update for the exact trigger signal
...
- [ ] KW-025: GIVEN a LEADER unit is alive at SS1-end (bonus snapshotted post-SS1) and is killed in SS4, WHEN SS6 resolves, THEN the ATK bonus remains active for all eligible family units in SS6 (fields persist until RESOLUTION ends)
- [ ] KW-026: GIVEN a LEADER unit is SILENCEd at the post-SS1 snapshot time, WHEN the bonus snapshot is computed, THEN the SILENCEd LEADER grants no bonus to family units for this RESOLUTION
- [ ] KW-039: GIVEN a LEADER is un-SILENCEd at snapshot time (bonus snapshotted post-SS1), then SILENCEd during SS3, WHEN SS6 resolves, THEN the snapshot bonus remains active; mid-RESOLUTION SILENCE does NOT retroactively invalidate a legally-taken snapshot
- [ ] KW-046: GIVEN a LEADER unit with HASTE is placed in SS1 of round R, WHEN the LEADER bonus snapshot is computed post-SS1 (after all APPEARANCE effects resolve), THEN the bonus IS applied this round — the LEADER is already on board when the post-SS1 snapshot fires; bonus is active from SS2 onward in round R
...
- [ ] `KeywordTriggered { payload: LeaderSnapshotTaken { leader_unit_id }, sub_step: 1 }` emitted post-SS1 (sub_step=1) for each LEADER whose bonus is applied
...
2. Collect all LEADER units: `has_keyword(SimpleKeyword::Leader)` + alive at SS1-end (HASTE LEADERs placed in SS1 ARE included — they are on the board when this runs)
...
**LEADER placed in SS1 (KW-046):** `leader_snapshot_system` runs AFTER SS1 fully resolves (post-SS1, pre-SS2). A LEADER entering in SS1 via HASTE or APPEARANCE IS on the board at snapshot time → bonus applies from SS2 onward in this same round.
