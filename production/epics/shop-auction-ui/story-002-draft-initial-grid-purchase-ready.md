...
- Do not drain `MessageReceiver<S2CGoldUpdate>` in Shop/Auction UI. Read the canonical shared own-economy presentation state populated by the single economy drain.
...
- First-session tooltip implementation, including exact placement, dismiss behavior, and persistence storage. The UX spec owns the non-occluding placement/persistence contract; implement it only in Story 009 or a separately scoped tooltip story.
