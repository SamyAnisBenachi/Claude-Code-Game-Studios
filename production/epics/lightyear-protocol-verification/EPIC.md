...
> **Status**: Ready
> **Stories**: 6 foundation stories + 2 Sprint 13 candidate hardening stories (S13-PROTO-INVARIANT-001, S13-PROTO-ORPHAN-DRAIN-001) authored as Sprint 13 candidates per PROMPT 803/804 (Story 007 Done; Story 008 Done) + 2 Sprint 18 candidate hardening stories (S18-PROTOCOL-SNAPSHOT-REAL-WIRE-TESTS-001 per PROMPT 1295 = story 009; S18-PROTOCOL-S2CACTIVATIONREJECTED-REGISTER-001 per PROMPT 1303 = story 010; both NOT activated)
...
| 009 | [S18-PROTOCOL-SNAPSHOT-REAL-WIRE-TESTS-001 — Real-Wire Snapshot Test Helper + 4 Test Migrations](story-009-s18-protocol-snapshot-real-wire-tests.md) | Logic (test infrastructure + 4 test migrations; NO production-code change) | Draft — future Sprint 18 candidate (per PROMPT 1295 from PROMPT 1287 §3.11 Lane W10 / §5 row SA-5; PROMPT 1202 F-08; PROMPT 1086 anti-pattern history), NOT activated | ADR-008, ADR-003, ADR-009, ADR-010, ADR-011, ADR-021 |
| 010 | [Register `S2CActivationRejected` + `ActivationRejectedReason` in Shared Protocol](story-010-s2c-activation-rejected-protocol-register.md) | Config/Data | Draft -- Sprint 18 candidate (S18-PROTOCOL-S2CACTIVATIONREJECTED-REGISTER-001 per PROMPT 1303), NOT activated | ADR-003, ADR-008 |
| 011 | [PlayerSnapshot.submitted Drop-vs-Wire Disposition](story-011-playersnapshot-submitted-disposition.md) | Decision-first (Path A drop / Path B wire / Path C defer) | Draft — Sprint 18 candidate (S18-PROTO-PLAYERSNAPSHOT-SUBMITTED-DISPOSITION-001 per PROMPT 1298 / authored by PROMPT 1305, integrated by PROMPT 1313 renumbered 009 → 011 to follow the 1306 wave-b 009 + the 1306 1303-reconcile 010 slots). Path B is HARD-BLOCKED on `S18-RSM-SUBMISSIONS-RECEIVED-CLEAR-001` (F-07 fix); Path A has no dependency. NOT activated | ADR-002, ADR-003, ADR-008 |

> Story sequence: 001 → 002 → 003 → 004; Story 005 is a follow-up protocol repair required before BLS-011 and HAND-UI-010. Story 006 is the first step in the spawn range prerequisite chain and unlocks BLS-012. Stories 007 and 008 are Sprint 13 candidate hardening stories sourced from `reports/PROMPT-803-MULTIPLAYER-RUNTIME-HARDENING-AUDIT-ROADMAP.md` §4 Lane A + §5 Must rows 1+2; both are authored as Sprint 13 candidates by PROMPT 804 and are NOT activated. Story 008 flips Story 007's invariant test to PASS; the two are intended to land together in the same Sprint 13 wave (or Story 007 lands with the test `#[ignore]`-gated then Story 008 removes the gate). Story 009 is a future Sprint 18 candidate hardening row sourced from `reports/PROMPT-1202-multiplayer-protocol-state-consistency-bug-audit.md` §2 row F-08 (with explicit historical PROMPT 1086 → PROMPT 1130 anti-pattern context); it is authored as a Sprint 18 candidate by PROMPT 1295 and is NOT activated. Story 010 is a Sprint 18 candidate authored by PROMPT 1303 from the PROMPT 1297 `C2SActivateCard` disposition audit; it registers the previously-missing `S2CActivationRejected` rejection message + `ActivationRejectedReason` enum in `shared/src/protocol.rs` so the rejection wire path exists ahead of the future server card-activation dispatcher epic. Story 010 is parallel-safe with Sprint 18 hand/fan/board lanes (touches `shared/src/protocol.rs` only) and does NOT close the broader `C2SActivateCard` half-wired gap on its own. Story 011 is a Sprint 18 candidate dead-state hygiene disposition sourced from `reports/PROMPT-1298-server-dead-state-hygiene-audit.md` §3 F-05; it is authored as a Sprint 18 candidate by PROMPT 1305, integrated by PROMPT 1313 with a 009 → 011 renumber to follow the 1306 wave-b 009 + 1303-reconcile 010 slots, and is NOT activated. Activation of any of these stories flips to Ready via the Sprint 18 plan + `/sprint-plan sprint-18` workflow.
