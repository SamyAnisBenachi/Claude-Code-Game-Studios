# Story 009: S18-PROTO-PLAYERSNAPSHOT-SUBMITTED-DISPOSITION-001 — Drop-vs-Wire Decision for `PlayerSnapshot.submitted`
...
3. *(Path A)* Quoted output of `grep -rn "\.submitted\b" client/src/
   ...
   evidence file `tests/evidence/lyp-story-009-playersnapshot-submitted-
   drop.md`.
4. *(Path B)* Quoted diff snippet of the HUD wire-up site (AC-B2); new
   ...
   exit status (AC-B4); evidence file `tests/evidence/lyp-story-009-
...
5. *(Path C)* Allowlist diff snippet (if invariant fixture exists);
   ...
   `tests/evidence/lyp-story-009-playersnapshot-submitted-defer.md`.
