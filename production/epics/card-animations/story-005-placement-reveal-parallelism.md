...
**Engine Notes**: ⚠️ **BLOCKED — two pre-implementation gates:**
1. **CA-3 gate**: `PlacementRevealAnimReady` payload schema must be defined in `board-rendering.md` before implementation. The payload must specify how Card Animations identifies the 5 unit entities (entity references, lane+cell indices, etc.).
2. **CA-21 gate**: `LaneCell { lane: u8, cell: u8 }` component must be defined in `board-rendering.md` AND `BoardLayout.cell_to_world(lane, cell)` method signature confirmed.
OQ-CA-01 (`AnimatorState` enum name) affects CA-3 and CA-21 `AnimatorState::Playing` assertions.
...
- Required: Mandatory simultaneous-start for placement reveal — all 5 lanes in single system pass, no `apply_deferred` between. `BoardLayout.cell_to_world()` as coordinate source for placement snap. `CardAnimationsSet::React.after(BoardRenderSet::ScheduleTweens)` ordering for same-frame `PlacementCancelAllAnimsRequested` delivery. PLACEMENT 250 ms hard cap applied at `Tween` construction: `duration.min(Duration::from_millis(250))`.
- Forbidden: Per-lane stagger during placement reveal (all 5 must start same frame). `Transform.translation` written directly while `Animator<Transform>` active. Any `*Bundle` type.
...
- [ ] **CA-3** — GIVEN `PlacementRevealAnimReady` is received with 5 lane entries, WHEN one `App::update()` tick runs, THEN all 5 unit entities have `Animator` components in `AnimatorState::Playing`. Implementation note: system must iterate all 5 entries in a single pass before any `apply_deferred` flush separates them — scheduling invariant validated in code review. **PRE-IMPLEMENTATION GATE: BLOCKED until `board-rendering.md` defines the `PlacementRevealAnimReady` payload schema.** **[BLOCKING]**
...
- [ ] **CA-21** — GIVEN a PLACEMENT-phase animation is in `AnimatorState::Playing` on an entity with `PlacementPhaseAnimator` marker and `LaneCell { lane: u8, cell: u8 }` component, WHEN `PlacementCancelAllAnimsRequested` is processed (same-frame delivery via SystemSet ordering), THEN (a) all `PlacementPhaseAnimator` entities are no longer in `Playing` state AND (b) `world.get::<Transform>(entity).unwrap().translation == board_layout.cell_to_world(lane_cell.lane, lane_cell.cell)`. **PRE-IMPLEMENTATION GATE: BLOCKED until `LaneCell` component and `BoardLayout.cell_to_world()` confirmed in `board-rendering.md`.** **[BLOCKING]**
...
1. **Simultaneous-start invariant (GDD Rule C-4):** `PlacementRevealAnimReady` handler iterates all entries in one pass. All 5 entity `Animator` insertions in the same `commands` batch before any flush. No `apply_deferred` between entries.
...
3. **`PlacementCancelAllAnimsRequested` handler (GDD Rule C-10, Edge Cases §2):** Queries `With<PlacementPhaseAnimator>`. For each entity: cancel `Animator` via `set_tweenable`; overwrite `Transform.translation` using `Res<BoardLayout>.cell_to_world(lane_cell.lane, lane_cell.cell)` from entity's `LaneCell` component. Same-frame delivery guaranteed by `CardAnimationsSet::React.after(BoardRenderSet::ScheduleTweens)`.
...
- [Story 003](story-003-simultaneous-track-animation.md): `Tracks<Transform>` for same-component simultaneous animations

---

## QA Test Cases

*PRE-IMPLEMENTATION GATE for CA-3, CA-21 — tests to be written once payload schema, `LaneCell`, and OQ-CA-01 resolve.*

**CA-3 — PlacementRevealAnimReady spawns Animators on all 5 entities in single pass (PRE-IMPL)**

- Given: World with `CardAnimationsPlugin`; 5 pre-spawned unit entities; `PlacementRevealAnimReady` message written with 5 entries (payload schema TBD — `board-rendering.md` gate)
- When: `app.update()` called once
- Then: All 5 entities have `Animator` components in `Playing` state
...
**CA-21 — PlacementCancelAllAnimsRequested cancels and snaps to cell position (PRE-IMPL)**

- Given: World with `CardAnimationsPlugin` + `BoardLayout` resource; 2 entities with `PlacementPhaseAnimator`, `LaneCell` (lane=1/cell=1; lane=2/cell=3), `Animator<Transform>` in `Playing`; `PlacementCancelAllAnimsRequested` message written
- When: `app.update()` called once
- Then: (a) no `PlacementPhaseAnimator` entity in `Playing`; (b) `Transform.translation == board_layout.cell_to_world(lane, cell)` for each entity
...
- Depends on: [Story 002](story-002-tween-cancel-replace-lifecycle.md) must be DONE; `board-rendering.md` `PlacementRevealAnimReady` payload schema defined; `LaneCell` component defined; OQ-CA-01 resolved
