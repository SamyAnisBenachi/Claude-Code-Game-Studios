...
**ADR Decision Summary**: When `timer_remaining_ms == 0` in LIVE_BIDDING, `auction_tick_system` transitions to RESOLVING synchronously. Case A (winner exists): release the winner's gold reservation with `api::release_gold_reservation(...)`, deduct the winning bid with `api::spend_gold(...)`, add card to hand, unicast `S2CCardAcquired`, broadcast `S2CAuctionSettled { winner: Some(...) }`, write `AuctionSettled` Bevy Message, return to IDLE. Case B (no bids): broadcast `S2CAuctionSettled { winner: None, amount: 0 }`, write `AuctionSettled`, return to IDLE.
...
- **Pre-implementation confirmation (Epic OQ4)**: Current Economy API confirmed in `server/src/core/economy/api.rs`: settlement uses `release_gold_reservation(economy, amount)` followed by `spend_gold(economy, amount)`; there is no reserved-spend wrapper API.
- **Pre-implementation confirmation (Epic gate 1)**: Protocol naming confirmed in `shared/src/protocol.rs`: `S2CCardAcquired.source` is `CardSource`, the auction variant is `CardSource::AuctionWon`, and `pub type AcquisitionSource = CardSource` preserves the GDD alias.

**Control Manifest Rules (Feature Layer)**:
- Required: Case A settlement must use Economy API calls only: verify `gold >= reserved_gold`, release the active reservation with `release_gold_reservation(...)`, then deduct the winning bid with `spend_gold(...)`; in release builds add an explicit `if player.gold < player.reserved_gold { log_critical_error!(...) }` guard before release.
...
- Forbidden: Do NOT call `release_gold_reservation(...)` or `spend_gold(...)` in Case B (no bids) — there is no reservation to release or winning bid to spend
- Note: ADR-013 is incorporated in control manifest v2026-05-01.

**Performance / No-Impact Note**:
- Settlement remains in Step 5 of `auction_tick_system` and completes synchronously in the same tick as LIVE_BIDDING expiry.
- Settlement is O(1): at most one winner economy lookup, one hand lookup, one unicast dispatch, one broadcast dispatch, and one internal `AuctionSettled` message.
- Target: `<0.1 ms` per auction tick for the settlement branch; no deferred resolution or multi-frame settlement state is allowed.
    ...
    match auction.current_leader {
        Some(winner) => {
            ...
            if let Some(econ) = economies.0.get_mut(&winner) {
                ...
                let reserved_gold = econ.reserved_gold;
                api::release_gold_reservation(econ, reserved_gold); // zeroes reservation
                if api::spend_gold(econ, bid_amount).is_err() {
                    tracing::error!("CRITICAL: auction settlement spend failed after reservation release");
                }
            }
            ...
            if let Some(hand) = hands.get_mut(&winner) {
                if hand.size < 10 {
                    hand.add(card_id);
                    // unicast S2CCardAcquired { card_id, source: CardSource::AuctionWon }
                } else {
...
Test: winner gets gold deducted and card added to hand
  ...
  Then: Player 1.gold == 3 (10 - 7)
        ...
        S2CCardAcquired { source: CardSource::AuctionWon } unicast to Player 1
  ...
  Edge cases: reserved_gold == 0 before settlement spend → debug_assert! fires (should not happen);
...
Test: hand-full at resolution — gold deducted, card NOT added
  ...
  Then: Player 1.gold == 3 (reservation released, then winning bid spent)
