...
$captureRoot = Join-Path $repoRoot "production\qa\evidence\captures\qa-cond-0007-resolution-replay"
$url = "http://127.0.0.1:$TrunkPort/"
$tracePath = Join-Path $captureRoot "qa-cond-0007-resolution-replay-trace.json"
$relativeCaptureRoot = "production/qa/evidence/captures/qa-cond-0007-resolution-replay"
...
function Get-HarnessReportJson {
    $response = Send-CdpCommand -Method "Runtime.evaluate" -Params @{
        expression = "JSON.stringify(globalThis.__qaCond0007Replay?.report ?? null)"
        returnByValue = $true
        awaitPromise = $false
    }
...
function Invoke-HarnessStepRender {
    ...
    Send-CdpCommand -Method "Runtime.evaluate" -Params @{
        expression = "globalThis.__qaCond0007ReplayRender('$escaped')"
        returnByValue = $false
        awaitPromise = $false
    } | Out-Null
}
...
$stepOrder = @(
    @{ id = "replay-start"; file = "01-replay-start.png" },
    @{ id = "replay-mid-first-sub-step"; file = "02-replay-mid-first-sub-step.png" },
    @{ id = "replay-second-sub-step"; file = "03-replay-second-sub-step.png" },
    @{ id = "replay-final-sub-step-buffered"; file = "04-replay-final-sub-step-buffered.png" },
    @{ id = "replay-drained-next-phase"; file = "05-replay-drained-next-phase.png" },
    @{ id = "recovery-snapshot-requested"; file = "06-recovery-snapshot-requested.png" }
)

$profileDir = Join-Path ([IO.Path]::GetTempPath()) ("qa-cond-0007-replay-cdp-" + [Guid]::NewGuid().ToString("N"))
$trunk = $null
$chrome = $null
$script:Socket = $null
$script:NextCdpId = 0
...
try {
    ...
    $trunkStartInfo.Arguments = ConvertTo-ArgumentString -Arguments @(
        "serve",
        "qa-cond-0007-resolution-replay-harness.html",
        "--address",
        "127.0.0.1",
        "--port",
        "$TrunkPort"
    ...
    do {
        if ($trunk.HasExited) {
            throw "Trunk exited before serving QA-COND-0007 replay harness. ExitCode=$($trunk.ExitCode)"
        }
        try {
    ...
    if ((Get-Date) -ge $trunkDeadline) {
        throw "Trunk did not serve QA-COND-0007 replay harness within $ReadyTimeoutSeconds seconds."
    }
    ...
    do {
        ...
        if (-not [string]::IsNullOrWhiteSpace($reportJson) -and $reportJson -ne "null") {
            $stateReady = Send-CdpCommand -Method "Runtime.evaluate" -Params @{
                expression = "globalThis.__qaCond0007Replay?.ready_for_capture === true"
                returnByValue = $true
                awaitPromise = $false
            }
    ...
    if ([string]::IsNullOrWhiteSpace($reportJson) -or $reportJson -eq "null") {
        throw "QA-COND-0007 replay harness did not publish a report within $ReadyTimeoutSeconds seconds."
    }

    $report = $reportJson | ConvertFrom-Json
    if ($report.verdict.readyForCapture -ne $true) {
        throw "QA-COND-0007 replay harness reported readyForCapture=false."
    }
    ...
    $result = [ordered]@{
        ...
        scope = "Resolution replay readability"
        url = $url
        viewport = [ordered]@{ width = 1366; height = 768 }
        uiScale = "100%"
        inputMethod = "deterministic ECS replay script against BR-006 queue and phase-buffering infrastructure"
        ...
        tracePath = "$relativeCaptureRoot/qa-cond-0007-resolution-replay-trace.json"
        capturedAt = (Get-Date).ToUniversalTime().ToString("o")
    }

    $result | ConvertTo-Json -Depth 100 | Set-Content -LiteralPath $tracePath -Encoding ascii
