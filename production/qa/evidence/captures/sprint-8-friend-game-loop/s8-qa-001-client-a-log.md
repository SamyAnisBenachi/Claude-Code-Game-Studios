...
Full browser/native Client A GUI execution was not completed in this Codex
...
windows or browser clients through the full route. The nearest covered Client A
evidence is the controlled real-Lightyear host route below.

Observed host protocol route:

```text
C2SHello
S2CHandshake
C2SCreateRoom
S2CRoomCreated(room_code=<ROOM_CODE_REDACTED>)
S2CSlotUpdated
C2SSelectClass(Iop)
C2SConfirmClass(Iop)
S2CClassLocked
...
C2SPlaceBid(amount=<SERVER_PRICE_PLUS_ONE>)
S2CAuctionBidAccepted
S2CAuctionSettled(winner=host)
S2CCardAcquired(source=AuctionWon)
S2CPhaseChanged(DraftShop)
C2SSignalReady(retract=false)
S2CPhaseChanged(Placement)
C2SSubmitPlacement(placements=[<SERVER_HAND_CARD>])
