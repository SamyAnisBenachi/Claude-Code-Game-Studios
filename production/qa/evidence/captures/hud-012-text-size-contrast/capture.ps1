param(
    [string]$Url = "http://127.0.0.1:8082/hud-text-size-contrast-harness.html?fixture=hud_012_text_size_contrast",
...
$relativeOutputDir = "production/qa/evidence/captures/hud-012-text-size-contrast"
$outputDir = Join-Path $repoRoot ($relativeOutputDir -replace "/", "\")
$summaryPath = Join-Path $outputDir "hud-012-text-size-contrast-capture-summary.json"
$summaryRelativePath = "$relativeOutputDir/hud-012-text-size-contrast-capture-summary.json"
...
function Get-HarnessState {
    $response = Send-CdpCommand -Method "Runtime.evaluate" -Params @{
        expression = "JSON.stringify(globalThis.__hud012TextSizeContrastEvidence ?? null)"
        returnByValue = $true
        awaitPromise = $false
    }
...
function Capture-Viewport {
    ...
    Send-CdpCommand -Method "Runtime.evaluate" -Params @{
        expression = "globalThis.__hud012TextSizeContrastEvidence = null"
        returnByValue = $true
    } | Out-Null
    ...
    if ($null -eq $state -or $state.ready_for_capture -ne $true) {
        $timeoutBaseName = "hud-012-text-size-contrast-${Width}x${Height}-timeout"
        $timeoutScreenshot = Join-Path $outputDir "$timeoutBaseName.png"
        Capture-Screenshot -Path $timeoutScreenshot
        throw "HUD-012 harness did not become ready at ${Width}x${Height}; title=$(Get-RuntimeValue -Expression 'document.title')"
    }

    Start-Sleep -Seconds 1

    $baseName = "hud-012-text-size-contrast-${Width}x${Height}"
...
$profileDir = Join-Path ([IO.Path]::GetTempPath()) ("hud-012-cdp-" + [Guid]::NewGuid().ToString("N"))
$chrome = $null
$script:Socket = $null
$script:NextCdpId = 0
...
try {
    ...
    $result = [ordered]@{
        ...
        verdict = [ordered]@{
            ...
            colorCaptureCount = 2
            fixtureValues = "phase=DRAFT_AUCTION, round=9, own=11g (4r), opponent=8g (3r), mana=6 / 10, reserve=+2 reserve"
        }
