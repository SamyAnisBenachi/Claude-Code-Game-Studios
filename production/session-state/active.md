...
- Test Evidence: `cargo test -p client --test shop_auction_ui_auction_activation_test` passed 6/6. Requested regressions passed: `shop_auction_ui_plugin_scaffold_formulas_test` 8/8, `shop_auction_ui_draft_initial_grid_test` 9/9, `shop_auction_ui_shop_panel_test` 8/8, and `presentation_plugin_scaffold_test` 3/3. `cargo check -p client` and `git diff --check` passed. `cargo fmt -p client -- --check` passed in the clean SAU-004 worker worktree; the root checkout command currently fails only on unrelated dirty `client/src/presentation/board_rendering.rs` formatting.
