...
>
> **R4 changes (2026-04-30):** OQ-BR-06 resolved — `C2SRequestSnapshot` now defined in `network-protocol.md` NP-43; stale text removed from 3 locations; BR-18c/BR-EC-STUCK ungated. `SpriteAlphaLens` API corrected: `set_alpha()` → `with_alpha()`. Player Fantasy absolute ceiling claim removed (no single absolute ceiling — scales with destroy count). Draw call worst-case corrected: cell nodes = 6 batches (not 3), status icons add 3–5; true worst case ~18–25. F4 theoretical 5-destroy ceiling corrected to 18,000ms. Rate-limit (NP-43) documented in all 3 recovery paths. Rule 14 priority tiers added (Tier 1: SHIELDED/TAUNT/STEALTH/IMMUNE always visible). 2 missing ACs added: BR-EC-PLACEMENT-STUCK, BR-CAMERA-PROJ. OQ-BR-10 closed (Approach A confirmed). ~7 BLOCKING items resolved.
>
...
*Pillar alignment: "No idle spectating" applies to PLACEMENT and DRAFT phases where decisions are live. RESOLUTION is the deliberate watch-the-tape phase — kept tight (≤5s default; per-round ceiling 6–13s depending on destroy count — see F4) so the watch never becomes structural dead time. The veteran-fatigue trade-off is acknowledged above. "Simple surface" — the visual rule is that positions are facts: one rule, infinitely deep.*
...
struct AnimQueue {
    ...
    total_duration_ms_cached: u32,  // computed via F4 at queue construction; use total_duration_ms() method
}

impl AnimQueue {
    // Returns precomputed F4 total; does not require a running App (pure read of cached field)
    fn total_duration_ms(&self) -> u32 { self.total_duration_ms_cached }
}
```
...
**`SpawnHighlightState`** — presentation-side component attached to every `BoardCellNode` entity:
...
Board Rendering seeds this state from `PlayerSnapshot.spawn_range_cells` during `S2CGameSnapshot` rebuild and updates it from `ResolutionEvent::SpawnRangeChanged { player_id, new_spawn_range_cells }` entries in `S2CResolutionEvent`. It is visual state only. The client must not derive spawn range from `ObjectiveDestroyed.was_fake` and must not expect a replicated `SpawnRange` ECS component.
...
**Custom `bevy_tweening` lens for sprite alpha.** `bevy_tweening` ships with `TransformPositionLens`, `TransformRotationLens`, `TransformScaleLens` — but no `Sprite.color.alpha` lens. The reveal tween (Rule 7), unit-death fade, ghost-fade-on-deselect, and any other alpha-driven sprite tween require a custom `SpriteAlphaLens` implementing `Lens<Sprite>` that assigns `sprite.color = sprite.color.with_alpha(lerp_value)` on the component directly (`Color` has no mutating `set_alpha()` method in Bevy 0.18). This lens is a deliverable of the reveal-tween implementation story; subsequent alpha tweens reuse it.
...
**Rule 1 — Client-side only.** Board Rendering contains no game logic, no validation, and no authoritative state. It reads Lightyear-replicated ECS components and consumes reliable S2C messages to drive visual output. It never sends C2S **game-logic** messages. The sole exception is `C2SRequestSnapshot`: Board Rendering may send this recovery message on protocol desync or stuck-state (EC-RESOLUTION-REVEAL-STUCK, EC-SUBSTEP-OOR, BR-18c). See OQ-BR-06 for the protocol contract.
...
**Draw call batching caveat — color tinting (R3 2026-04-30):** In Bevy 0.18, sprites with different `Sprite.color` values do NOT batch even when sharing the same `Handle<Image>`. During PLACEMENT: cell nodes render in 2–3 tint states (idle cyan, spawn-active gold, dim/inactive) = 2–3 draw calls for cell nodes instead of 1. HP bar fill bars render in 3 tint states (Green/Yellow/Red per F2) = up to 3 draw calls instead of 1. **Expected worst-case breakdown (all five lanes active, PLACEMENT state):**

| Source | Draw calls | Notes |
|---|---|---|
| Board background | 1 | Standalone atlas |
| Field washes (0–5) | 0–5 | Standalone, one per active lane |
| Cell nodes — 6 tint states | 6 | 3 functional states (idle/active/dim) × 2 player-half tints (cool/warm); Sprite.color splits each into a separate batch |
| Unit atlas opaque (own + opponent) | 1 | Single atlas; all at alpha=1.0 |
| Unit atlas translucent (reveal-tween, alpha<1) | 1 | Separate translucent batch |
| HP bar fill bars — 3 tint states | 3 | Green/Yellow/Red; tint-split even within unit atlas |
| Board-elements atlas (objectives, tokens, status) | 1–2 | May split by tint for objectives vs. non-objectives |
| Ghost unit | 1 | Semi-transparent (alpha=0.5) |
| Status icons — N tint states (R4) | 3–5 | Per-keyword color tinting on board-elements atlas splits each keyword type into a separate batch |
| **Estimated worst case** | **~18–25** | Exceeds the 15-call ceiling at full PLACEMENT state; see remediation note below |

The draw call ceiling IS exceeded at full PLACEMENT state (R4 corrected: 6 cell-node tint batches + 3–5 status-icon keyword batches push worst case to ~18–25 calls). The 15-call ceiling is a design target, not an enforced gate; AC BR-3 measures against it at ship time. If measurements exceed an acceptable threshold (technical-director decision), remediation options: (a) bake tint variants into separate atlas frames instead of using Sprite.color tinting; (b) encode spawn-highlight as a 1-px overlay child sprite rather than a cell-node tint; (c) use a grayscale base + overlay approach for HP bars.
...
**HP bar update invariant (R3 2026-04-30 — required for Approach A safety):** No `Animator<Transform>` is EVER scheduled on the HP bar fill entity's `Transform.scale` axis. HP bar fill width is driven exclusively by `UpdateHpBars` polling `Changed<UnitStats>` — never by a tween. If any `Animator<Transform>` were added to the fill entity's scale axis, both `UpdateHpBars` (in `BoardRenderSet::UpdateHpBars`) and `bevy_tweening`'s `component_animator_system` (in `BoardRenderSet::TickAnimations`) would write `scale.x` in the same frame, producing visual glitches. The VFX table phrase "scale.x lerp to new value" means the Lightyear replication cadence provides a natural per-tick smoothing — it is NOT a tween directive. See AC BR-HP-INVARIANT.
...
**ResolutionReveal stuck-state recovery.** If `S2CPlacementReveal` was received but `S2CResolutionEvent` does not arrive within `resolution_reveal_timeout_ms` (default 2000ms — server crash mid-resolution, lost message), the client sends `C2SRequestSnapshot` (defined in `network-protocol.md` NP-43; OQ-BR-06 resolved) and resets `BoardRenderState` to whatever the snapshot delivers. This is the only message-loss fallback. **Rate-limit (NP-43):** The server silently ignores `C2SRequestSnapshot` if a snapshot was delivered within the last `snapshot_cooldown_ms` (5000ms). If the request is dropped, Lightyear's 30s heartbeat-disconnect grace is the intentional last-resort backstop — accepted trade-off for friend-game scope.
...
**Display priority ordering (R4 2026-04-30):** When more than 3 effects are active on a unit, visible slots (max 3) are filled in tier order, then remaining effects enter the `+N` overflow:

| Priority tier | Keywords | Reason |
|---|---|---|
| **Tier 1 — always visible first** | SHIELDED, TAUNT, STEALTH, IMMUNE | Combat-deciding — hiding these causes incorrect placement decisions (e.g. attacking a SHIELDED unit wastes damage) |
| **Tier 2 — fill remaining slots, then overflow** | HASTE, POISON, STUN, BURN, SLOW, and all other keywords | Relevant but not directly placement-deciding in the same way |

Within each tier, effects are ordered by descending remaining-duration (longest-remaining fills the earliest slot). `keyword-system.md` must define a `display_tier: u8` field (1 or 2) per keyword; Board Rendering reads this at runtime to fill the priority order. Board Rendering must NOT hard-code keyword names — it reads `display_tier` from the keyword definition.

**AC for priority ordering:** See BR-STATUS-CONTRACT (existing, covers 4-effect case). Tier enforcement is an implementation invariant; a Tier-1 keyword must always appear in slot 0–2 even if a Tier-2 keyword arrived first.

**Cross-doc dependency:** the actual icon-to-keyword mapping and `display_tier` values are owned by `keyword-system.md`. Board Rendering provides the layout slots and priority logic; the keyword GDD provides the icons and tier assignments.
...
| **Card Data & Pool** | Pool → Rendering | Card art `TextureAtlasLayout` + `Image` handles loaded at startup; per-card atlas index looked up by `card_id` at unit spawn time; Board Rendering reads the card definition to select the correct `TextureAtlas { layout, index }` runtime struct for the spawned `Sprite` |
...
- **Maximum (all sub-steps + 2 destroys, tuning ceiling): 400 + 800 + 6×(1000+300) + 2×(800+1000) = 12,600ms ≈ 12.6s** (R3 corrected: prior entry said 11,400ms due to arithmetic error)
- Theoretical with 5 destroys at ceiling knob values (extremely rare — all objectives lost in one round): `400 + 800 + 6×(1000+300) + 5×(800+1000) = 18,000ms ≈ 18s` — exceeds the 12.6s 2-destroy ceiling. The "+N more destroyed" summary for N_destroyed > 2 is deferred polish (R4: not a hard enforced cap; actual observed ceiling depends on round frequency of 3–5 destroy rounds).
...
**Player Fantasy ceiling (R4 revised):** The "≤5s default" framing is the primary design anchor (typical rounds, no destroys). Rounds with 1–2 objective destroys run 6.5–13s at ceiling knobs. Rounds with 3–5 destroys (extremely rare) can reach 14–18s at ceiling knobs. There is no single absolute ceiling — the watch length scales with the round's dramatic content. Veterans-fatigue trade-off acknowledged in Player Fantasy. The ≤5s default ceiling defends against structural dead time in ordinary rounds; extended rounds are proportional to significant game events (objective destructions) which are inherently dramatic.
...
**EC-SUBSTEP-OOR — If `S2CResolutionEvent` contains a `sub_step` value outside [1–6]:** treat as a fatal protocol desync. Discard the entire `AnimQueue` (clear all groups), clear `PendingResolutionScript` if present, log error containing `sub_step` value, and request a fresh `S2CGameSnapshot` from the server via `C2SRequestSnapshot` (defined in `network-protocol.md` NP-43). Do NOT silently skip the group — silent skip corrupts subsequent state references because surrounding sub-steps assume the dropped group's outputs (HP changes, position changes, gold awards) were applied. Snapshot recovery is clean. This aligns with network-protocol.md client contract for protocol violations. **Rate-limit (NP-43):** If the server silently ignores the request (within `snapshot_cooldown_ms` = 5000ms of a prior snapshot), Lightyear's 30s heartbeat-disconnect is the intentional last-resort backstop.
...
**EC-RESOLUTION-REVEAL-STUCK — If `S2CPlacementReveal` was received but `S2CResolutionEvent` does not arrive within `resolution_reveal_timeout_ms` (default 2000ms):** request a fresh `S2CGameSnapshot` via `C2SRequestSnapshot` (defined in `network-protocol.md` NP-43; OQ-BR-06 resolved) and reset `BoardRenderState` to whatever the snapshot delivers. This is the only message-loss fallback; without it the client is permanently stuck on a revealed-but-static board with no input. **Rate-limit (NP-43):** If the server silently ignores the request (within `snapshot_cooldown_ms` = 5000ms of a prior snapshot), Lightyear's 30s heartbeat-disconnect grace is the intentional last-resort backstop.
...
**EC-PLACEMENT-STUCK — If `PendingResolutionScript` is populated (S2CResolutionEvent stored) but `S2CPlacementReveal` NEVER arrives** (server crash during PLACEMENT): after `resolution_reveal_timeout_ms` (default 2000ms) of `Time<Virtual>` with `PendingResolutionScript = Some(_)` in `Placement` state, send `C2SRequestSnapshot` (defined in `network-protocol.md` NP-43; OQ-BR-06 resolved) and reset `BoardRenderState` to whatever the snapshot delivers. Clear `PendingResolutionScript` on snapshot receipt. **Rate-limit (NP-43):** If the server silently ignores the request within `snapshot_cooldown_ms` (5000ms), Lightyear's 30s heartbeat-disconnect is the intentional last-resort backstop. See AC BR-EC-PLACEMENT-STUCK.
...
**EC-OBJ-MISS — If `ObjectiveDestroyed` arrives for a lane where no objective entity currently exists on the client** (replication removed it before the reliable message was processed): suppress the destruction VFX and log a warning. Do not spawn a temporary entity — this risks double-reveal if the replicated entity arrives late. Spawn range highlights change only if a separate `SpawnRangeChanged` entry for the affected attacker is present in the same `S2CResolutionEvent`; missing objective visuals must not cause Board Rendering to infer spawn range.
...
| **Network Protocol** (Approved) | Hard | `S2CPlacementReveal` → opponent entities arrive via Lightyear replication + reveal tween; `S2CResolutionEvent` → animation queue; `S2CPhaseChanged` → all `BoardRenderState` transitions; `S2CGameSnapshot` → full board rebuild on connect/reconnect; **`C2SRequestSnapshot`** (defined in `network-protocol.md` NP-43; resolves OQ-BR-06) → client desync recovery; server rate-limits to once per `snapshot_cooldown_ms` (5000ms) |
| **Card Data & Pool** (Approved) | Hard | Unit `Image` + `TextureAtlasLayout` assets loaded at startup; per-card atlas index looked up by `card_id` at unit spawn time; fallback to `ui_unit_placeholder` atlas frame if `card_id` is missing (EC-CARD-MISS) |
| **Game Config** (Approved) | Hard | `lane_count=5` and `cells_per_lane=8` confirm board grid dimensions at startup; animation timing constants (`board_sub_step_duration_ms`, `board_pre_anim_pause_ms`, `board_inter_step_pause_ms`, `board_objective_reveal_hold_ms`, `board_obj_reveal_anim_ms`, `board_unit_reveal_tween_ms`, `board_unit_reveal_start_scale`, `board_reveal_timeout_ms`, `board_obj_id_reconnect_timeout_ms`) and visual tuning (`board_cell_width`, `board_lane_height`, `board_hp_green_threshold`, `board_hp_red_threshold`, `board_co_occupancy_offset`, `board_prism_spin_speed`) loaded from `GameConfig` resource. R2 (2026-04-29) removed `board_fog_opacity` and `board_fog_lift_ms`; added the reveal-tween + reconnect-timeout knobs. |
...
| `unit_reveal_tween_duration_ms` (R2 new) | `board_unit_reveal_tween_ms` | 250 ms | 150–400 | Reveal feels abrupt; dramatic moment lost | Reveal sluggish; players wait too long |
| `unit_reveal_tween_start_scale` (R2 new) | `board_unit_reveal_start_scale` | 0.4 | 0.3–0.6 | Reveal pop too subtle | Units feel like they're growing from a dot — cartoonish |
| `resolution_reveal_timeout_ms` (R2 new) | `board_reveal_timeout_ms` | 2000 ms | 1500–5000 | Hair-trigger on slow networks; false-positive snapshot requests | Stuck-state too long; player perceives game frozen |
| `objective_identities_reconnect_timeout_ms` (R2 new) | `board_obj_id_reconnect_timeout_ms` | 5000 ms | 3000–10000 | Surfaces server bugs quickly but may misfire on slow reconnect handshake | Player blocked too long without diagnostic |
| `objective_reveal_anim_ms` (R2 new — formalised) | `board_obj_reveal_anim_ms` | 800 ms | 500–1000 | Reveal feels rushed | Momentum killed |
...
| `co_occupancy_side_offset` | `board_co_occupancy_offset` | 8.0 px | 4–16 | Units nearly overlap in 2v2 | Units clip outside cell node. **Constraint (intake-validated):** `co_occupancy_side_offset + sprite_w/2 ≤ cell_width/2` — at min `cell_width=48` and `sprite_w=48`, max offset is 0; at default `cell_width=64`, max offset is 8. Misconfiguration logs warning + clamps to safe range. |
...
|---|---|---|
| `HP_THRESHOLD_EPSILON` | `1e-4_f32` | F2 floating-point boundary tolerance — see F2 example for `3/10` case |
| `UNIT_SPRITE_WIDTH` | `48.0_f32` | Fixed art constant for unit sprite width in world units. Used in F3 co-occupancy constraint: `co_occupancy_side_offset + UNIT_SPRITE_WIDTH/2 ≤ cell_width/2`. Source: unit art spec. If art produces a different width, update this constant and re-validate the constraint. |

**Knob guard policy (R2 2026-04-29):** All `GameConfig` knobs in this section are intake-clamped (out-of-range value → clamp to safe-range bound + warn) **except** `health_bar_green_threshold` / `health_bar_red_threshold` which are intake-asserted (out-of-range or inverted → `assert!` panic at config load — game refuses to start with bad HP threshold config). The asymmetry: HP thresholds drive a player-visible state machine (Green/Yellow/Red); silently clamping them would mask config errors. Visual-only knobs (fog-replaced reveal tween, durations) clamp silently.
...
**REMOVED in R2 2026-04-29.** The fog overlay system was replaced by server-side replication filtering of newly-placed entities + a 250ms reveal tween. No fog assets, no fog sprites, no fog Z layer. See Rule 7.

#### Unit Sprite Fallback

Board Rendering requires one fallback atlas frame for EC-CARD-MISS (missing card_id):
- `ui_unit_placeholder_48x64` — solid color tile + "?" glyph. Prevents render-loop panics on asset miss.

#### Reserved Atlas Frames (R2 new)

**`hp_bar_white_pixel_1x2`** — a 1×2 white-pixel frame reserved in the **unit atlas** (not board-elements). HP bar children use this frame so they share the unit atlas's `Handle<Image>` and batch with units in a single draw call (Rule 5, AC BR-3a). Background bar = white pixel tinted with `Sprite.color = Color::srgba(0.1, 0.1, 0.1, 0.7)` (dark grey background); fill bar = white pixel tinted to Green/Yellow/Red per F2. `Transform.scale.x` drives fill width.
...
| **Opponent unit reveal — simultaneous appearance** | `S2CPlacementReveal` | Each newly-replicated opponent entity scales 0.4×→1.0× + alpha 0→1 over 250ms, all starting same frame (synchronized) | Simple | **BLOCKING** | **Sharp "ink-stamp" thud + chord sting (per the prior fog-lift sting; no longer 'veil lift')** |
...
**Must ship real art for M2 (BLOCKING):** cell nodes (idle + spawn-active), objective unknown sprite, objective real-reveal sprite, unit base rings (both shapes), lane number labels, trap face-down tile, **opponent unit reveal tween** (scale + alpha + audio sting per Rule 7), unit slide tweens, unit death fade, status-effect icon slots (Rule 14 — board layout slots; icon glyphs owned by keyword-system.md).
...
1. **Reveal-tween chord sting** (replaces former "fog lift sting" — same audio cue, different trigger now bound to opponent units appearing) — without this, the simultaneous reveal loses its drama.
...
| Opponent unit reveal tween (R2 replacement for fog) | **Board Rendering** | On `S2CPlacementReveal`, each newly-replicated opponent entity gets a 250ms scale+alpha tween via `bevy_tweening` `TransformScaleLens` + project-local `SpriteAlphaLens`. No fog overlay exists. |
| Spawn range highlights | **Board Rendering** | Seeds cell-node highlight state from `PlayerSnapshot.spawn_range_cells` on snapshot rebuild, then recolors `BoardCellNode` sprites from `ResolutionEvent::SpawnRangeChanged` entries in `S2CResolutionEvent` |
...
| BR-3 | GIVEN a **fully-populated board** defined as: 5 lanes, 4 units per lane (2 per side, co-occupant pairs in 2v2 mode), all 5 objective entities standing with HP bars, prism token at lane-3 spawn cell, all 5 lanes Field-washed, no traps face-down, **opponent has just appeared post-reveal-tween (alpha=1.0, scale=1.0)** so the worst-case live state is captured — WHEN the frame is rendered in a live WASM browser build, THEN total GPU draw call count is ≤15. Evidence: browser devtools performance panel screenshot in `production/qa/evidence/`. | ADVISORY |
| BR-3a | GIVEN a fully-populated board after spawn, WHEN all entities with a unit `Sprite` are queried, THEN every such entity carries (a) the same `Handle<Image>` `AssetId` for `Sprite.image` (single unit atlas image) AND (b) a `texture_atlas: Some(TextureAtlas { layout, .. })` whose `layout` is the same `Handle<TextureAtlasLayout>` `AssetId` (single unit atlas layout). When all board-rendering sprite entities are queried, exactly 2 distinct `Handle<Image>` `AssetId`s are present across atlased sprites (unit atlas image + board-elements atlas image; standalone-atlas sprites like background, Field washes count separately and are tracked in BR-3). **R2 note:** the type is NOT `Handle<TextureAtlas>` — that asset type does not exist in Bevy 0.18. | BLOCKING |
...
| BR-3c (R2 new) | GIVEN any HP bar child entity in the World, WHEN inspected, THEN: (a) it carries the same `Handle<Image>` `AssetId` as unit sprites (atlas-shared via `hp_bar_white_pixel_1x2` reserved frame); (b) `texture_atlas` field references the unit atlas layout with `index = HP_BAR_WHITE_PIXEL_INDEX`. HP bars must batch with units in a single draw call. | BLOCKING |
...
| BR-6 (R2 rewritten) | GIVEN `BoardRenderState == Placement`, WHEN the World is queried, THEN **zero fog sprite entities exist** (the fog overlay was removed in R2 2026-04-29). Cell nodes, lane labels, Field washes, standing objectives, and prior-round persistent units/traps/structures on the opponent's half are all visible (fully alpha=1.0, no overlay sprite obscuring them). | BLOCKING |
| BR-7 (R3 rewritten — collect-then-reveal buffer) | GIVEN `BoardRenderState == Placement` with no opponent newly-replicated entities yet, WHEN `S2CPlacementReveal` is received and `App::update()` runs, THEN: (a) on the first tick after `S2CPlacementReveal`, opponent entities are collected into a buffer (no animators yet — buffer fill frame); (b) on the SECOND tick, every collected opponent entity has an `Animator<Transform>` (scale lens) AND an `Animator<Sprite>` (custom `SpriteAlphaLens`) in `AnimatorState::Playing` — all firing simultaneously regardless of when Lightyear delivered each entity; (c) initial scale = 0.4, initial alpha = 0.0 ±0.01; (d) after injecting `unit_reveal_tween_duration_ms` (250ms) of `Time<Virtual>` delta and converging, every collected opponent entity has `Transform.scale == Vec3::splat(1.0)` ±0.01 AND `Sprite.color.alpha == 1.0` ±0.01; (e) local player's own newly-placed entities do NOT enter the buffer and do NOT get reveal-tween animators. NOTE: verify the exact Lightyear 0.26 API for detecting newly-replicated opponent entities before implementing — "or equivalent" is not sufficient specification; cross-reference `liv-bevy-lightyear`. | BLOCKING |
...
| BR-10 (R2 corrected) | GIVEN no ghost preview entity or marker exists for `card_id`, WHEN `GhostPlacementChanged { target: None, card_id: Some(card_id) }` is received, THEN no panic occurs, no entity is spawned, and World state is unchanged. The despawn path uses `if let Ok(mut ec) = commands.get_entity(e) { ec.despawn(); }` (Bevy 0.18: `commands.get_entity` returns `Result<EntityCommands, EntityDoesNotExistError>`, NOT `Option`); the `if let Some(...)` form does NOT compile in 0.18 and must not be used. Marker-component variants are removed if present, no-op if absent. | BLOCKING |
...
| BR-12 (R2 type-tightened) | GIVEN a `S2CResolutionEvent` with sub_step values [1, 1, 3, 3, 3, 5] (events themselves are opaque placeholders pending OQ-BR-03), WHEN the animation queue is built, THEN `AnimQueue.groups.len() == 3` sorted ascending [1, 3, 5] with event counts [2, 3, 1] respectively; `AnimQueue.total_duration_ms()` returns a `u32` value **exactly** equal to `unit_reveal_tween_duration_ms + pre_animation_pause_ms + 3*(resolution_sub_step_duration_ms + inter_step_pause_ms)` (no tolerance — all duration arithmetic uses `u32` millisecond integers, no f32 accumulation). The `AnimQueue` Resource exposes `total_duration_ms()` as a callable method without requiring a running `App`. | BLOCKING |
| BR-13 (R2 updated) | GIVEN `N_groups=0` (empty `S2CResolutionEvent`) and zero opponent entities replicated this round, WHEN the queue is built, THEN `AnimQueue.groups.is_empty()`, `AnimQueue.total_duration_ms() == pre_animation_pause_ms` (no reveal-tween cost since no opponent entities appeared), and the state machine still runs `pre_animation_pause_ms` of pause before transitioning `ResolutionExecuting` → `ResolutionObjectiveReveal`. No sub-step Tweens are spawned during this transition. (Reveal tween case is covered by BR-7.) | BLOCKING |
...
| BR-17 | GIVEN the board is in any state with N entities, WHEN `S2CGameSnapshot` is received, THEN: (a) all prior board entities are despawned; (b) rebuilt entity count and component values match snapshot data exactly; (c) zero `Animator<Transform>` and zero `Animator<Sprite>` components from the prior state remain in the World after rebuild (verify via World query over all `Animator<*>`); (d) `AnimQueue`, `PendingPhaseChange`, `PendingResolutionScript` resources are all cleared. Implementation note: despawn commands are queued via `Commands` and not applied until `apply_deferred` — the rebuild (spawn) system MUST run in a separate system after an explicit `apply_deferred` flush, or both despawn and spawn must be in the same system. The test verifies entity count AFTER the second `App::update()` call (the first flushes the despawns; the second confirms the rebuilt state). | BLOCKING |
| BR-18a (R2 split) | GIVEN `S2CGameSnapshot { phase: RESOLUTION }` is received and `S2CObjectiveIdentities` has NOT yet been re-received, WHEN `App::update()` is called 3 times, THEN `BoardRenderState == Reconnecting` (NOT `DraftShop`) — the system holds, and a warning containing "Reconnecting" is logged on the first tick. No phase advancement. | BLOCKING |
| BR-18b (R2 split — happy path) | GIVEN `BoardRenderState == Reconnecting`, WHEN `S2CObjectiveIdentities` is subsequently inserted as a Message and `App::update()` runs, THEN `BoardRenderState == DraftShop` and the `ObjectiveIdentityCache` is populated with one entry per replicated objective. | BLOCKING |
| BR-18c (R2 new — timeout; R4 ungated) | GIVEN `BoardRenderState == Reconnecting` and `objective_identities_reconnect_timeout_ms` (5000ms default) of `Time<Virtual>` has elapsed without `S2CObjectiveIdentities`, WHEN the system runs, THEN: (a) a `C2SRequestSnapshot` is enqueued (per NP-43); (b) an error containing "S2CObjectiveIdentities timeout" is logged; (c) state remains `Reconnecting`. Backoff schedule (5s, 10s, 20s, 30s ceiling) prevents permanent block. Note: if request is silently rate-limited by server (NP-43), next backoff attempt is the recovery path. | BLOCKING |
| BR-19 (R3 rewritten — poison-entity technique for TypeId detection) | GIVEN 5 objective entities in the World in any state prior to `ObjectiveDestroyed`, WHEN inspected, THEN: (a) every objective entity carries a `Sprite` component with `texture_atlas: Some(TextureAtlas { layout, index })` where `index == env_objective_unknown_64x96_index`; (b) every objective entity carries the same set of allowed component types and NO others — verified by querying for an explicit positive list and asserting count matches; (c) zero entities carry a banned identity component. **Poison-entity technique required for (c):** for each banned type (`RealObjective`, `FakeObjective`, `ObjectiveIdentity`, `IsKnown`, `IsTrue`), insert the component on a scratch entity to register its `ComponentId` (guaranteeing `World::components().get_id::<T>() == Some(_)`), then assert no objective entity carries that `ComponentId`, then despawn the scratch entity. Without this poison step, `get_id::<T>()` returns `None` if the banned type was never registered — and `None.is_none()` trivially passes even when the invariant is not enforced. This is the runtime ADR-001 isolation invariant; the poison step is the load-bearing correctness mechanism. | BLOCKING |
| BR-20 | GIVEN `ObjectiveDestroyed { was_fake: false, lane: 3 }` is received during `ResolutionObjectiveReveal`, THEN: (a) after `App::update()` with 0ms time advance, the lane-3 objective entity is still present and shows the unknown frame; (b) after injecting `objective_reveal_hold_ms` (500ms) of `Time<Virtual>` delta, a real-reveal marker is present on the objective entity (atlas frame swapped to `env_objective_real_reveal_64x96` OR a `RevealFlash` marker component is attached); (c) after injecting the flash animation duration (~300ms) plus buffer, the objective entity is despawned; (d) no spawn range highlight component changes occur unless a separate `SpawnRangeChanged` entry is present in the same `S2CResolutionEvent`. | BLOCKING (reclassified from ADVISORY 2026-04-30 — testable headlessly with `Time<Virtual>` injection) |
...
| BR-Z-LOCAL | GIVEN a unit entity at `Transform.translation.z = Z_UNITS (3.0)` with an HP bar child entity, WHEN the HP bar child's `Transform` is inspected, THEN its `translation.z` is `0.1` (LOCAL — added to parent's Z to produce global Z=3.1=Z_HEALTH_BARS). The HP bar child's local Z must NOT equal `Z_HEALTH_BARS` directly (would render at global Z=6.1, above ghost preview). | BLOCKING |
| BR-EC-EARLY (R2 split + corrected) | GIVEN `BoardRenderState == Placement` and no `S2CPlacementReveal` has arrived, WHEN `S2CResolutionEvent` arrives and `App::update()` runs, THEN: (a) `BoardRenderState` remains `Placement`; (b) `PendingResolutionScript == Some(_)`; (c) a warning containing "S2CResolutionEvent before reveal" is logged. | BLOCKING |
| BR-EC-EARLY-CONSUME (R2 new — split from above) | GIVEN `PendingResolutionScript == Some(_)` and `BoardRenderState == Placement`, WHEN `S2CPlacementReveal` arrives and `App::update()` runs, THEN: (c1) within the same tick, opponent reveal-tween animators exist on newly-replicated entities (per BR-7); (c2) `BoardRenderState == ResolutionReveal`; (c3) `PendingResolutionScript == None` (cleared on consumption); (c4) **`pre_animation_pause_ms` IS still applied** after the reveal tween completes (R2 design decision — preserves Player Fantasy beat); after `unit_reveal_tween_duration_ms + pre_animation_pause_ms` of injected `Time<Virtual>`, `BoardRenderState == ResolutionExecuting`. | BLOCKING |
...
| BR-SPAWN-HIGHLIGHTS | GIVEN `S2CGameSnapshot.PlayerSnapshot.spawn_range_cells` seeds both players' current spawn range, WHEN the board rebuilds from snapshot, THEN every `BoardCellNode` has `SpawnHighlightState` matching the owning player's current valid placement cells. GIVEN a later `S2CResolutionEvent` contains `SpawnRangeChanged { player_id, new_spawn_range_cells }`, WHEN the event is processed after its preceding `ObjectiveDestroyed`, THEN only that player's affected `BoardCellNode` highlight states update to the new range and persist through subsequent DRAFT/PLACEMENT frames. | BLOCKING |
| BR-EC-OBJMISS | GIVEN no objective entity exists for lane 3 (replicated removal arrived before reliable message), WHEN `ObjectiveDestroyed { lane: 3, was_fake: false }` arrives, THEN: (a) no new entity is spawned; (b) spawn range highlight components on `CellNode` entities do not change unless a separate `SpawnRangeChanged` entry is present; (c) no panic; (d) a warning is logged. World entity count is unchanged. | BLOCKING |
| BR-EC-STUCK (R2 knob-tied; R4 ungated) | GIVEN `BoardRenderState == ResolutionReveal` and `resolution_reveal_timeout_ms` (default 2000ms) has elapsed in `Time<Virtual>` without `S2CResolutionEvent`, WHEN the system runs, THEN a `C2SRequestSnapshot` message is enqueued (per NP-43) AND a warning containing "ResolutionReveal stuck" is logged. After the resulting `S2CGameSnapshot` arrives, the board rebuilds per Rule 11. Note: if the server silently rate-limits the request (NP-43), Lightyear's 30s heartbeat-disconnect is the intentional backstop. | BLOCKING |
| BR-EC-LOBBY-SNAPSHOT (R2 new) | GIVEN `BoardRenderState == Lobby` (zero board entities), WHEN `S2CGameSnapshot` is received with `phase == DRAFT_INITIAL`, THEN: (a) board entities are spawned matching snapshot data; (b) `AnimQueue`, `PendingPhaseChange`, `PendingResolutionScript` resources remain in cleared state (no leftover state); (c) `BoardRenderState` transitions to `DraftInitial`; (d) no panic. Same single-tick rule as BR-17. | BLOCKING |
| BR-INTERSTEP-PAUSE (R2 new) | GIVEN 2 `AnimGroup`s queued (sub-steps 1 and 3), WHEN group 1's `resolution_sub_step_duration_ms` of `Time<Virtual>` advances and `App::update()` runs, THEN: (a) `BoardRenderState` is still `ResolutionExecuting`; (b) no group-2 tweens have started (verify via World query — no `Animator<*>` with the group-2 events' targets); (c) after additional `inter_step_pause_ms` advance, group-2 tweens are now active. Verifies the inter-step pause is a discrete frame-stable phase, not just total-time math. | BLOCKING |
| BR-STATUS-CONTRACT (R2 new — Rule 14) | GIVEN a unit entity with a `StatusEffectsList` carrying 4 active effects, WHEN the status-icon-update system runs, THEN: (a) exactly 3 `StatusIcon` child entities exist on the unit (max-3-visible per Rule 14); (b) one `StatusOverflowBadge` child entity exists in the 4th slot showing "+1"; (c) all 4 children are positioned in a horizontal stack at top-right of unit sprite (positions match Rule 14 layout offsets ±0.5 px); (d) all 4 children carry the board-elements atlas (no third atlas). | BLOCKING |
| BR-STATUS-TIER (R5 new — Rule 14 display priority) | GIVEN a unit with 4 active effects where 1 is Tier-1 (`display_tier=1`, e.g. SHIELDED) and 3 are Tier-2 (`display_tier=2`, e.g. HASTE, POISON, BURN), WHEN the status-icon-update system runs **regardless of the insertion order of effects in `StatusEffectsList`**, THEN: (a) the Tier-1 effect (SHIELDED) occupies slot 0 (top-right icon position); (b) exactly 2 Tier-2 effects occupy slots 1 and 2 (ordered by descending remaining-duration within the tier); (c) exactly 1 `StatusOverflowBadge` exists showing "+1"; (d) inserting SHIELDED last into the `StatusEffectsList` must produce identical slot assignments as inserting it first — tier order is not insertion-order-dependent. Board Rendering reads `display_tier` from the keyword definition at runtime and MUST NOT hard-code keyword names. | BLOCKING |
| BR-STATUS-COOCCUPANCY (R2 new — Rule 14) | GIVEN a unit at co-occupancy index 1 (X offset = +4.0 from cell center) with 1 active status effect, WHEN status icon position is computed, THEN the icon's world-space X equals `unit_world_x + (unit_w/2 - 8.0)` — i.e., the icon inherits the parent unit's X offset (it does NOT center on the cell). | BLOCKING |
| BR-RECONNECT-CACHE-CLEAR (R2 new) | GIVEN `ObjectiveIdentityCache` is populated with 5 entries, WHEN `S2CGameSnapshot` is received and Rule 11 rebuild runs, THEN `ObjectiveIdentityCache` is empty (cleared) and `BoardRenderState == Reconnecting` until `S2CObjectiveIdentities` arrives. | BLOCKING |
| BR-SYSTEMSET-ORDER (R3 updated — Rule 13) | GIVEN the `BoardRenderPlugin` is added to a test `App` AND `app.update()` has been called at least once (to initialize the schedule graph), WHEN system schedule is inspected via `app.world().resource::<Schedules>()`, THEN: (a) `BoardRenderSet` enum variants are configured in `Update` schedule with `chain()` ordering `ReadMessages → ResolveStateMachine → SpawnEntities → ScheduleTweens → UpdateHpBars → TickAnimations`; (b) `bevy_tweening` `component_animator_system` runs in `BoardRenderSet::TickAnimations`. **Implementation note:** `app.update()` MUST be called before schedule inspection — an uninitialized schedule graph has no nodes and the ordering assertion would pass vacuously. | BLOCKING |
| BR-2-ATLAS (R2 type-corrected) | GIVEN a fully-populated board with units, objectives, cell nodes, prism token, status-effect icons, and HP bars (post-reveal-tween, no fog), WHEN all `Sprite.image` `Handle<Image>` `AssetId`s on board-rendering atlased entities are collected (excluding standalone-atlas sprites: background, Field wash), THEN exactly **2** distinct `AssetId<Image>`s are present (unit atlas image + board-elements atlas image). Status-effect icons share the board-elements atlas (Rule 14). A third atlas indicates a regression. | BLOCKING |
| BR-HP-INVARIANT (R3 new — Rule 6) | GIVEN any unit entity present on the board with `hp_current > 0` during `ResolutionExecuting`, WHEN the World is inspected after `ScheduleTweens` runs, THEN the HP bar fill child entity carries zero `Animator<Transform>` components. This invariant ensures that `UpdateHpBars`'s direct `scale.x` write and `TickAnimations`' tween advance cannot conflict on the same field in the same frame. Violation indicates a tween was scheduled on the fill entity's scale axis — a bug, not a feature. | BLOCKING |
| ~~BR-FOG-OPACITY~~ (R2 REMOVED) | ~~Fog entity initialization~~ — fog removed in R2 2026-04-29; AC no longer applicable. Replaced by knob-guard policy in Tuning Knobs section. | — |
| BR-HP-THRESHOLD-INVERT (R2 new) | GIVEN a `GameConfig` with `health_bar_red_threshold = 0.6` and `health_bar_green_threshold = 0.5` (inverted), WHEN config is loaded at startup, THEN the load assertion panics with a message containing both threshold values. Uses `assert!`, not `debug_assert!`. Verified via `#[should_panic(expected = "HP threshold config invalid")]`. | BLOCKING |
| BR-HP-EPSILON (R2 new) | GIVEN F2 with default thresholds (green=0.6, red=0.3) and `HP_THRESHOLD_EPSILON = 1e-4`, WHEN F2 is called with `hp_current=3, hp_max=10` (f32 ratio 0.29999...), THEN bar_color == Yellow (epsilon-aware comparison correctly classifies the boundary case as "at-or-above red"). Without the epsilon this test is the only way to detect the boundary regression. | BLOCKING |
| BR-COOCC-CONSTRAINT (R2 new) | GIVEN a `GameConfig` with `co_occupancy_side_offset = 16.0` and `cell_width = 48.0` (constraint violation: 16 + 24 > 24 → unit clips outside cell), WHEN config loads, THEN `co_occupancy_side_offset` is clamped to `(cell_width - sprite_width) / 2 = 0.0` and a warning is logged containing both values. | BLOCKING |
| BR-EC-PLACEMENT-STUCK (R4 new — from EC-PLACEMENT-STUCK edge case) | GIVEN `PendingResolutionScript = Some(_)` and `BoardRenderState == Placement`, WHEN `resolution_reveal_timeout_ms` of `Time<Virtual>` elapses without `S2CPlacementReveal` arriving, THEN: (a) a `C2SRequestSnapshot` message is enqueued (per NP-43); (b) a warning containing "PendingResolutionScript stuck" is logged; (c) `PendingResolutionScript` is NOT cleared until snapshot receipt. Note: if server rate-limits the request (NP-43), Lightyear heartbeat-disconnect is the backstop. | BLOCKING |
| BR-CAMERA-PROJ (R4 new — from OQ-BR-02) | GIVEN the `BoardRenderPlugin` is added to an `App` and `app.update()` has run at least once, WHEN the camera entity is queried, THEN: (a) exactly one entity with `Camera2d` marker exists; (b) `OrthographicProjection` is present on that entity; (c) no entity carries a perspective projection (`PerspectiveProjection`); (d) `Transform.translation.z` is positive (camera sits above the 2D plane). | BLOCKING |
...
- BR-2, BR-2b, BR-4, BR-13, BR-22, BR-22b, BR-HP-EPSILON are pure function tests — no ECS `World` needed.
- BR-7, BR-14, BR-15, BR-18a/b/c, BR-20, BR-21, BR-EC-STUCK, BR-INTERSTEP-PAUSE, BR-EC-EARLY-CONSUME require time-stepped ECS via `Time<Virtual>` injection. Test scaffolding: use `App::new().add_plugins(MinimalPlugins)` (which includes `TimePlugin`) — bare `World::new()` does NOT propagate `Time<Virtual>` into the default `Time<()>` alias.
- BR-3, BR-FRAME-TIME, BR-RECONNECT-TIME require live WASM browser builds with GPU profiling.
...
- BR-2b, BR-22b, BR-HP-THRESHOLD-INVERT use `#[should_panic(expected = "...")]` to verify panic-on-bad-input — a missing annotation means a panicking test would be marked failed instead of passed.
- BR-7 reveal tween test inserts a fixture `Replicated`-tagged entity in the same tick as `S2CPlacementReveal` to simulate Lightyear's spawn behavior; a real Lightyear test harness is not required.

## Open Questions

**OQ-BR-01 — Sang Méprise suppression signal (RESOLVED 2026-05-18 — ADR-024)**
~~The fake reveal sequence has a "confirmed reveal" variant (no surprise sting) when the Sang Méprise ability was active this round — the attacker already knew the identity. Board Rendering needs a signal indicating this. The delivery mechanism is undefined: replicated component, a field in `S2CResolutionEvent`, or a dedicated S2C message.~~ **Final resolution:** the `ObjectiveIdentityCache` resource (defined in Data Structures) IS the suppression signal. The audio-suppression branch reads `ObjectiveIdentityCache.identities` directly: if an entry exists for `(defender_player_id, destroyed_lane)`, the standard reveal tween still plays in full (500ms hold + reveal animation per `objective-system.md` Rule 6) but the surprise audio sting and any "surprise" particle layer are suppressed. No new wire field, no new replicated component, no new S2C message. Cache lifecycle (populated from live `S2CSangMepriseReveal` or `S2CGameSnapshot.active_sang_meprise_reveals`; cleared on phase change to non-RESOLUTION) is fully specified in ADR-024 §5; suppression-signal rationale in ADR-024 §6; Card Animations Edge Case wording in ADR-024 §7. The follow-on implementation story `S18-PROTO-SANG-MEPRISE-DRAIN-001` wires the `MessageReceiver<S2CSangMepriseReveal>` drain and removes the protocol allowlist row at `tests/invariants/protocol_completeness_test.rs:296-309`.
*Owner: ~~Network Protocol GDD + Keyword System GDD~~ → ADR-024. Status: RESOLVED — answered via existing Data Structures (`ObjectiveIdentityCache`) without protocol or replication change.*
...
**OQ-BR-04 — Spawn range update signal (RESOLVED 2026-05-05)**
~~Recommended: derive from `ObjectiveDestroyed.was_fake == true`.~~ ~~R3 resolution: replicate a `SpawnRange { cells }` component.~~ **Final resolution:** Board/Lane System owns the authoritative live spawn range projection in `SpawnRangeState`. Objective System owns objective destruction facts/counters only. Board Rendering receives spawn range through two protocol paths:

1. **Recovery/reconnect:** `PlayerSnapshot.spawn_range_cells` in `S2CGameSnapshot`.
2. **Live update:** `ResolutionEvent::SpawnRangeChanged { player_id, new_spawn_range_cells }` inside the ordered reliable `S2CResolutionEvent`, after the corresponding `ObjectiveDestroyed` entry.

Do not use snapshot-only for connected clients, because the board must update without forcing a reconnect/snapshot request. Do not add or consume a replicated `SpawnRange` component unless future docs explicitly reverse this decision.
*Owner: Board/Lane System GDD + Network Protocol GDD + Board Rendering GDD. Status: RESOLVED — implementation split into NP-006 → BLS-012 → BR-011 prerequisite chain.*
...
**OQ-BR-06 — `C2SRequestSnapshot` contract (RESOLVED R4 2026-04-30)**
`C2SRequestSnapshot` is now defined in `network-protocol.md` NP-43 with: empty payload `{}`; valid in all IN_GAME phases; server responds with `S2CGameSnapshot` unicast; rate-limited at `snapshot_cooldown_ms` (default 5000ms — server silently ignores within cooldown). ACs BR-EC-STUCK, BR-18c, and BR-24 are ungated. Cross-doc GDD text and dependency table updated R4.

*Owner: Network Protocol GDD. Status: RESOLVED — implementation may proceed. Rate-limit behavior documented in all board-rendering.md recovery paths.*
...
**OQ-BR-08 — Visual Identity Anchor compliance (OPEN — refreshed R2 2026-04-29)**
The Visual/Audio Requirements section specifies render layers, reveal-tween timings (250ms scale + alpha), idle oscillation amplitudes, but does not reference the project's Visual Identity Anchor. If the anchor (defined in `design/gdd/game-concept.md` or art bible) says e.g. "everything reads as physical paper", then the reveal-tween easing curve, unit oscillation curves, and objective reveal flash all have aesthetic constraints not captured here. Must be cross-referenced once the art bible is authored. (R2 note: fog tween references replaced with reveal tween — same OQ scope, updated wording.)
*Owner: Art Director + Art Bible. Blocking: M2 VFX polish stories (M3 if pushed).*

**OQ-BR-09 — Fake reveal audio cue (PROVISIONALLY RESOLVED R2 2026-04-29 — "trickster chord" is the design position; final pass is audio-director's call)**
Current spec for fake reveal: replace the "hollow dud thud" wording with a **"trickster chord"** — a brief, slightly off-key musical sting that frames the moment as cleverness rather than malfunction. Communicates "this was a bluff" without taking sides emotionally (attacker hears "outsmarted, but it was a clever bluff"; defender hears "my misdirection landed"). Final tonal character (mode, instrument, length) is the audio-director's call at first audio pass. The Visual/Audio Requirements VFX table audio cue text MUST be updated when audio-director closes this. Non-blocking for M2 implementation; the visual sequence (500ms hold + crack overlay + FAKE text) is unchanged.
*Owner: Audio Director (audio-director gate). Non-blocking for M2 implementation. Status: design-direction locked, audio character pending.*

**OQ-BR-10 — HP bar live-update implementation: poll vs tween (RESOLVED R4 2026-04-30 — Approach A)**
**Approach A selected:** frame-poll via `Changed<UnitStats>` filter — `UpdateHpBars` system reads the replicated component each frame and writes `Transform.scale.x` directly on the HP bar fill entity. Rule 6 HP bar update invariant enforces that no `Animator<Transform>` is ever scheduled on the fill entity's scale axis (see BR-HP-INVARIANT AC). The "lerp" in the VFX table description refers to the natural per-tick replication cadence, not a tween directive. Approach B (per-event tween) was rejected: adds ~50 Animator components per resolution batch and conflicts with the HP bar update invariant. `BoardRenderSet::UpdateHpBars` documentation updated to note "poll-based Approach A."

*Owner: Board Rendering GDD. Status: RESOLVED — Approach A baked into Rule 6, Rule 13, and BR-HP-INVARIANT AC.*
