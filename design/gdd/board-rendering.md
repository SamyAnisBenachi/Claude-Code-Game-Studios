...
> **Status**: Designed — /design-review 2026-04-30 R3 NEEDS REVISION applied in-session. Re-review recommended in fresh session after OQ-BR-06 lands in network-protocol.md.
...
*Pillar alignment: "No idle spectating" applies to PLACEMENT and DRAFT phases where decisions are live. RESOLUTION is the deliberate watch-the-tape phase — kept tight (≤5s default, **≤12.6s absolute ceiling** including objective-reveal sequence — see F4) so the watch never becomes structural dead time. The veteran-fatigue trade-off is acknowledged above. "Simple surface" — the visual rule is that positions are facts: one rule, infinitely deep.*
...
| Cell nodes — 3 tint states | 3 | Idle/active/dim colors; tint-split batches |
...
| **Estimated worst case** | **~12–17** | Borderline at the 15-call ceiling in worst case |

The draw call ceiling MAY be exceeded at full PLACEMENT state. If measurements (AC BR-3) exceed 15, the tinting strategy must be revisited — for example, encoding spawn-highlight as a separate 1-px overlay child sprite rather than a tint on the cell node itself, or consolidating HP bar colors using a grayscale-with-overlay approach.
...
**Player Fantasy ceiling: ≤12.6s absolute (single RESOLUTION worst case, R3 corrected from ≤11.5s).** The "≤5s default" framing is preserved for typical rounds (no destroys). Veterans-fatigue trade-off acknowledged in Player Fantasy. F4 revised value: 12,600ms = 400+800+6×(1300)+2×(1800) at ceiling. Going beyond 12.6s requires a Player Fantasy revisit.
...
**EC-PLACEMENT-STUCK — If `PendingResolutionScript` is populated (S2CResolutionEvent stored) but `S2CPlacementReveal` NEVER arrives** (server crash during PLACEMENT): after `resolution_reveal_timeout_ms` (default 2000ms) of `Time<Virtual>` with `PendingResolutionScript = Some(_)` in `Placement` state, request a fresh `S2CGameSnapshot` via `C2SRequestSnapshot` and reset `BoardRenderState` to whatever the snapshot delivers. Clear `PendingResolutionScript` on snapshot receipt. Lightyear's 30s heartbeat-disconnect is the backstop if `C2SRequestSnapshot` is also lost. Add BLOCKING AC: "GIVEN `PendingResolutionScript = Some(_)` and `BoardRenderState == Placement`, WHEN `resolution_reveal_timeout_ms` elapses without `S2CPlacementReveal`, THEN `C2SRequestSnapshot` is enqueued and a warning is logged." (Gated: OQ-BR-06)
...
| BR-18c (R2 new — timeout) (**GATED: OQ-BR-06** — requires `C2SRequestSnapshot` in network-protocol.md before implementation) | GIVEN `BoardRenderState == Reconnecting` and `objective_identities_reconnect_timeout_ms` (5000ms default) of `Time<Virtual>` has elapsed without `S2CObjectiveIdentities`, WHEN the system runs, THEN: (a) a `C2SRequestSnapshot` is enqueued; (b) an error containing "S2CObjectiveIdentities timeout" is logged; (c) state remains `Reconnecting`. Backoff schedule (5s, 10s, 20s, 30s ceiling) prevents permanent block. | BLOCKING |
...
**OQ-BR-06 — `C2SRequestSnapshot` contract (OPEN — ESCALATED R2 2026-04-29 to BLOCKING for implementation)**
Rule 11's stuck-state recovery (EC-RESOLUTION-REVEAL-STUCK), Rule 9's fatal sub_step desync handler (EC-SUBSTEP-OOR), Rule 11's reconnect S2CObjectiveIdentities timeout, and ACs BR-EC-STUCK / BR-18c / BR-24 all require a `C2SRequestSnapshot` message. **This message is currently NOT defined in `network-protocol.md`** (verified 2026-04-29 R2 review — the C2S table contains `C2SHello`, `C2SPurchaseCard`, `C2SRefreshShop`, `C2SActivateCard`, `C2SSignalReady`, `C2SPlaceBid`, `C2SSubmitPlacement`, `C2SAcknowledgeResult`, `C2SHeartbeat` — no RequestSnapshot). Server's silent-discard rule for unknown C2S messages means recovery requests would vanish with no error log. **Required additions to `network-protocol.md`:**
- New `C2SRequestSnapshot` message (no payload required; or `{ reason_code: u8 }` for telemetry)
- Server response: re-send `S2CGameSnapshot` reflecting current state (NOT cached snapshot — must include post-resolution state if `phase == RESOLUTION` per Rule 11 invariant)
- Rate-limit policy on server side (e.g., max 1 RequestSnapshot per second per client)
- Add to valid C2S phase table for ALL phases (recovery is universally allowed)

*Owner: Network Protocol GDD. **Status: BLOCKING — Board Rendering implementation of EC-RESOLUTION-REVEAL-STUCK, EC-SUBSTEP-OOR, BR-EC-STUCK, BR-24, BR-18c CANNOT begin until `C2SRequestSnapshot` is defined in NP GDD.** This is a cross-doc dependency that should be resolved in a parallel revision of `network-protocol.md`.*
