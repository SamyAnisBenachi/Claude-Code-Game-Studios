...
| BR-7 (R2 rewritten — reveal tween) | GIVEN `BoardRenderState == Placement` with no opponent newly-replicated entities yet, WHEN `S2CPlacementReveal` is received and `App::update()` runs, THEN: (a) within the same tick, every newly-replicated opponent entity (detected via `Added<Replicated>` filter or equivalent on the spawn frame) has an `Animator<Transform>` (scale lens) AND an `Animator<Sprite>` (custom `SpriteAlphaLens`) in `AnimatorState::Playing`; (b) initial scale = 0.4, initial alpha = 0.0 ±0.01; (c) after injecting `unit_reveal_tween_duration_ms` (250ms) of `Time<Virtual>` delta and converging, every such entity has `Transform.scale == Vec3::splat(1.0)` ±0.01 AND `Sprite.color.alpha == 1.0` ±0.01; (d) the **local player's** newly-placed entities do NOT have reveal-tween animators (they were already on screen). | BLOCKING |
...
| BR-18c (R2 new — timeout) | GIVEN `BoardRenderState == Reconnecting` and `objective_identities_reconnect_timeout_ms` (5000ms default) of `Time<Virtual>` has elapsed without `S2CObjectiveIdentities`, WHEN the system runs, THEN: (a) a `C2SRequestSnapshot` is enqueued; (b) an error containing "S2CObjectiveIdentities timeout" is logged; (c) state remains `Reconnecting`. Backoff schedule (5s, 10s, 20s, 30s ceiling) prevents permanent block. | BLOCKING |
| BR-19 (R2 rewritten — no `inspect_entity` dependency) | GIVEN 5 objective entities in the World in any state prior to `ObjectiveDestroyed`, WHEN inspected, THEN: (a) every objective entity carries a `Sprite` component with `texture_atlas: Some(TextureAtlas { layout, index })` where `index == env_objective_unknown_64x96_index`; (b) every objective entity carries the same set of allowed component types: `{Sprite, Transform, GlobalTransform, ChildOf, ObjectiveHp, BoardPosition, Visibility, ViewVisibility, InheritedVisibility, ...standard Bevy render components}` and NO others — verified by querying for an explicit positive list and asserting count matches; (c) zero entities anywhere in the World carry a component whose `TypeId` matches `RealObjective`, `FakeObjective`, `ObjectiveIdentity`, `IsKnown`, or `IsTrue` markers (test enumerates these `TypeId`s explicitly via `World::components().get_id::<T>()` for each banned type — no string introspection required). This is the runtime ADR-001 isolation invariant. | BLOCKING |
...
| BR-SYSTEMSET-ORDER (R2 new — Rule 13) | GIVEN the `BoardRenderPlugin` is added to a test `App`, WHEN system schedule is inspected, THEN: (a) `BoardRenderSet` enum variants are configured in `Update` schedule with `chain()` ordering `ReadMessages → ResolveStateMachine → SpawnEntities → ScheduleTweens → UpdateHpBars → TickAnimations`; (b) `bevy_tweening` `component_animator_system` runs in `BoardRenderSet::TickAnimations`. Verified via `App::world().resource::<Schedules>()`. | BLOCKING |
