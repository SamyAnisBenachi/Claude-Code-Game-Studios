...
**OQ-BR-04 — Spawn range update signal (RESOLVED R3 2026-04-30)**
~~Recommended: derive from `ObjectiveDestroyed.was_fake == true`.~~ **R3 resolution: this approach is architecturally incorrect.** Spawn range is computed by Economy System (formula: `spawn_range = min(1 + fakes_destroyed × fake_objective_spawn_advance, 3)`) and owned by Board/Lane System. Deriving it from `ObjectiveDestroyed` events in Board Rendering bypasses the Economy System and breaks if any card effect or mechanic grants bonus spawn advance. **Correct resolution:** Economy System replicates a `SpawnRange { cells: u8 }` component per player (one entity per player, updated whenever spawn range changes). Board Rendering reads via `Changed<SpawnRange>` in `UpdateHpBars` (or a dedicated system in `ReadMessages`). This is the same pattern as `ObjectiveHp` replication. No message type needed. Board/Lane System or Network Protocol GDD must define the `SpawnRange` component and its replication scope (all clients, unreliable channel).
*Owner: Board/Lane System GDD + Economy System GDD. Status: design decision made; implementation requires SpawnRange component added to board-lane-system.md replication table.*
