...
> **Status**: In Review — /design-review pass 2 revised 2026-04-30. Re-review recommended in fresh session.
> **Author**: User + Agents
...
- Lanes 1/2/4/5: spell card dropped. Prism state is still updated (`collected = true`). `S2CPrismRewardDropped { player_id: PlayerId, lane: u8 }` sent via reliable unicast to the owning player (pending NP GDD registration — see Dependencies). No `S2CCardAcquired` broadcast. No refund. No queue.
...
**Rule 9 — Respawn timing.** At the END of `resolve_prism_draws`, after all reward broadcasts have been issued, for each player where `pending_respawn[player] == true`: set `collected[lane][player] = false` for all 5 lanes. Reset `pending_respawn[player] = false`. Emit `S2CPrismRespawned { player_id: PlayerId }` via reliable unicast to both players (pending NP GDD registration — see Dependencies). The `PrismPresence` component replication (unreliable channel, per network-protocol.md) propagates per-lane token visibility on the next frame; `S2CPrismRespawned` guarantees respawn delivery even if the unreliable frame is dropped.
...
| **Network Protocol** | Approved | Hard | `S2CCardAcquired { card_id, source: PrismLane{1..5} }` reliable unicast on each successful hand add; `PrismPresence { collected: bool }` component replication | Source enum already exists (line 396–400). **NP GDD status:** (1) ✓ `PrismBoardState.player_id` confirmed present in NP GDD R5 revision; (2) ✓ `GoldAwardReason::PrismReward` removed per Rule 11 / OQ3 resolved; (3) ⚠ NP OQ1 (Lightyear 0.26 unicast API for `S2CCardAcquired`) still open — blocks Prism epic start. **New messages pending NP GDD registration:** `S2CPrismRespawned { player_id: PlayerId }` (reliable unicast to both players on full-set respawn — Rule 9/10) and `S2CPrismRewardDropped { player_id: PlayerId, lane: u8 }` (reliable unicast to owning player on hand-full drop — Rule 7). |
...
| **Card Acquisition** | Approved | None | Explicitly bypassed (per `card-acquisition.md` line 80) | Hand-mutation API (which Card Acquisition owns internally) is shared — Prism System must call the same API to avoid dual-ownership of the hand vector. Exact API name is OQ1 (open — must resolve before Prism epic implementation). |
...
| PS-20 | **GIVEN** a player's unit collects a valid prism in any lane (hand not full), **WHEN** `resolve_prism_draws` processes the collection, **THEN** exactly one `S2CCardAcquired { card_id: [lane-appropriate card ID], source: PrismLane{L} }` message is staged for reliable unicast to the owning player — verifiable by reading a staging buffer resource from the `World` after `resolve_prism_draws` runs. **Implementation note:** if Lightyear 0.26 does not expose a unit-testable outbound buffer in `World`-based tests, this AC reclassifies as Integration evidence requiring an `App::new()` test per project integration standards (OQ1 resolution will clarify the correct test approach). (Note: `PrismCollected` is emitted by Board / Lane System, not the Prism System — its emission is tested in board-lane-system.md.) | BLOCKING |
