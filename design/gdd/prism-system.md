...
> **Status**: Designed — /design-review MAJOR REVISION applied 2026-04-30. Re-review recommended in fresh session.
> **Author**: User + Agents
...
| **Network Protocol** | Approved | Hard | `S2CCardAcquired { card_id, source: PrismLane{1..5} }` reliable unicast on each successful hand add; `PrismPresence { collected: bool }` component replication | Source enum already exists (line 396–400). **Required NP GDD fixes before Prism epic starts:** (1) `PrismBoardState` must add `player_id: PlayerId` field — current schema cannot reconstruct per-player prism state in the reconnect snapshot; (2) `GoldAwardReason::PrismReward` enum variant must be removed (prisms grant zero gold — the variant is a live footgun); (3) NP OQ1 (Lightyear 0.26 unicast API and channel type for `S2CCardAcquired`) must be resolved before any prism epic stories can be started. |
| **Game Config** | Approved | Soft | Two tuning knobs: `prism_strike_damage` (default 1) and `prism_strike_mana_cost` (default 3) — both read at session start | See Section G. Knobs in `assets/config/game_config.ron`. **Required fix:** both fields must be added to the `GameConfig` struct definition in `game-config.md` (currently absent from the struct). |
...
| PS-12 | **GIVEN** `resolve_prism_draws` receives a `PrismCollected` message for a (player, lane) pair whose token is already marked collected (stale duplicate), **WHEN** the message is evaluated, **THEN** the message is discarded: no reward granted, no seed consumed, no client error sent AND exactly one server-side warning log entry is written for the discarded duplicate (the discard is silent to clients but auditable on the server). | BLOCKING |
...
| PS-17 | **GIVEN** multiple players each have a unit eligible for prism collection in the same RESOLUTION (including at least one Lane 3 collection to generate audit log entries), **WHEN** `resolve_prism_draws` processes all collections, **THEN** the server-side audit log for that RESOLUTION contains entries in ascending `player_id` order, and within the same `player_id`, in ascending lane index order — determinism requirement per ADR-005 / server-rng.md Rule 6. | BLOCKING |
...
| PS-20 | **GIVEN** a player's unit collects a valid prism in any lane (hand not full), **WHEN** `resolve_prism_draws` processes the collection, **THEN** exactly one `S2CCardAcquired { card_id: [lane-appropriate card ID], source: PrismLane{L} }` message is enqueued for reliable unicast to the owning player — verifiable by inspecting the outbound message queue before frame flush. (Note: `PrismCollected` is emitted by Board / Lane System, not the Prism System — its emission is tested in board-lane-system.md.) | BLOCKING |
| PS-22 | **GIVEN** `prism_strike_damage` is set outside the documented safe range (< 1 or > 3) in `game_config.ron`, **WHEN** the server reads the config at startup, **THEN** the server process panics with an explicit message identifying the invalid field — no silent out-of-range value reaches gameplay. | ADVISORY |
| PS-23 | **GIVEN** a player has a `prism_strike` or `prism_reserve` card in hand AND the current phase is PLACEMENT or RESOLUTION, **WHEN** the player attempts to play the card, **THEN** the card-play validation system (Rule 13, owned by Card Acquisition / Round State Machine) rejects the play: no state mutation occurs, no mana is deducted, the card remains in hand, and no error message is sent to the client. | BLOCKING |
...
*24 total: 19 BLOCKING, 5 ADVISORY. Every Core Rule from Section C (R1–R13) maps to at least one criterion. PS-17 reclassified BLOCKING (determinism requirement). PS-20 rewritten (tests S2CCardAcquired output, not PrismCollected emission). PS-21 ordering note: ID is non-sequential by convention (added in revision); functionally follows PS-16.*
...
| OQ3 | **`GoldAwardReason::PrismReward` removal.** `network-protocol.md` line 383 contains a `PrismReward` variant in the `GoldAwardReason` enum. Prisms grant zero gold by design (Rule 11). This variant is a live footgun — implementers reading network-protocol.md will see it in scope and may wire it up accidentally. **Must be removed from network-protocol.md before any implementation PR is opened for the Prism epic.** | Network Protocol GDD | Open — MUST FIX BEFORE IMPLEMENTATION (not minor housekeeping) |
