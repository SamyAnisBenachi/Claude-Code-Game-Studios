...
| **Network Protocol / Lightyear** | Auction → Protocol | Broadcasts: `S2CAuctionCard` (before `S2CPhaseChanged`), `S2CAuctionBidAccepted`, `S2CAuctionSettled`. Unicasts: `S2CAuctionBidRejected` (bidder only), `S2CCardAcquired` (winner only). Receives: `C2SPlaceBid`. `S2CGoldBroadcast { player_id, gold, reserved_gold }` is fired by the Economy System on every auction-related gold mutation (`reserve_gold`, `release_gold_reservation`, `spend_reserved_gold`). The `reserved_gold` field is required so opponents can compute free gold (`gold - reserved_gold`) — the figure the Player Fantasy depends on. **Note:** `S2CGoldBroadcast` payload in network-protocol.md must include `reserved_gold: u32` — update required before auction implementation. |
...
| AU19 | **GIVEN** the Auction System is in SELECTING or RESOLVING, **WHEN** `AbortAuction` is received, **THEN** Auction System returns to IDLE, `reserved_gold == 0` for all players, and `AuctionSettled` Bevy Message is NOT fired. | BLOCKING |
| M7-a | **GIVEN** an accepted bid is processed, **WHEN** the Economy System fires `S2CGoldBroadcast` for the bidding player, **THEN** the broadcast payload includes both `gold: u32` and `reserved_gold: u32` for the affected player, so the receiving client can compute `free_gold = gold - reserved_gold`. *Requires `S2CGoldBroadcast` payload to include `reserved_gold` — update network-protocol.md first (see Interactions table note).* | BLOCKING |
...
| OQ7 | **Late-game wealth disparity can exclude the losing player from all auctions.** At 35g vs 10g, the wealthy player bids 11g and the poor player cannot contest any remaining auction. This violates "No idle spectating" for the economically losing player. Planned remediation path: consider a catch-up mechanism (e.g., bonus interest rate for the trailing player, or a consolation gold token on auction loss). This OQ is open; the current spec is not blocked by it but the mechanism should be designed before M3. | Game Designer | Before M3 |
| OQ8 | **No-bid frequency risk.** Late-game cards below ~6–7g effective value (considering opportunity cost vs. interest) may rationally receive no bids from both players. Pool burn is permanent. If playtesting reveals frequent no-bid auctions undermining the signature mechanic, consider a minimum-incentive mechanism (e.g., a gold bonus for placing the first bid on a no-bid card). Monitor in first playtest sessions. | Game Designer | After first playtest |
