...
| **Bid input** | **Preset buttons only (no free-form field):** "+1g" (= `current_price + 1`, minimum bid), "+3g" (= `current_price + 3`), "+5g" (= `current_price + 5`). Each button fires immediately on click — no separate Confirm step. Buttons are individually disabled when `free_gold < current_price + offset`. All three disabled when hand full or player is current leader. Bid size is the signal. Full spec: `design/gdd/shop-auction-ui.md` Section C Rules 4–5. |
