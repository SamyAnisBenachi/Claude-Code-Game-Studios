...
> **Status**: In Review
> **Author**: User + Agents
> **Last Updated**: 2026-04-30 (post-/design-review pass 3 revision: 14 blockers resolved)
...
1. Call `spend_reserved_gold(leader)`: `gold = gold.saturating_sub(reserved_gold); reserved_gold = 0`. This is not a `can_afford_bid` re-check — the reservation IS the commitment. **Implementer must `debug_assert!(player.gold >= player.reserved_gold)` before this subtraction** and log a critical server error if violated — under correct RSM phase isolation the invariant always holds, but a plain u32 subtraction silently produces ~4.29B gold in release builds on any upstream invariant violation.
...
AuctionSnapshot {
    ...
    last_accepted_bid:   u32,           // 0 if no bids placed yet. Client rule: if last_accepted_bid == 0, minimum valid bid = starting_price; else minimum valid bid = last_accepted_bid + 1.
...
| AU1-b | **GIVEN** the RSM enters DRAFT_AUCTION, **WHEN** `S2CPhaseChanged(DRAFT_AUCTION)` is dispatched, **THEN** an `S2CAuctionCard` message was already queued in the same or earlier frame. *BLOCKED pending NP Open Question 3 (Lightyear reliable channel FIFO guarantee across message types). **Resolution gate = the ADR-008 Validation Criteria integration test** proving Lightyear reliable-channel FIFO ordering holds across message types on the same channel. Implement as integration test once ADR-008's test confirms the guarantee. **Do NOT substitute a code-review assertion or Bevy schedule inspection** — those prove server enqueue order, not client receive order. If ADR-008's test reveals Lightyear does NOT guarantee cross-type FIFO, add explicit sequence numbers to `S2CAuctionCard` and `S2CPhaseChanged` and rewrite this AC to assert `S2CAuctionCard.seq < S2CPhaseChanged.seq`.* | BLOCKING — Integration (pending NP OQ3 / ADR-008) |
...
| AU16 | **GIVEN** an auction is LIVE_BIDDING and `bidder.gold.saturating_sub(bidder.reserved_gold) < bid_amount`, **WHEN** `C2SPlaceBid { amount: bid_amount }` is received, **THEN** `S2CAuctionBidRejected { reason: InsufficientGold }` is unicast to the bidder and no auction state changes. | BLOCKING |
...
| AU19-b | **GIVEN** the Auction System is in SELECTING, **WHEN** `AbortAuction` is received, **THEN** Auction System returns to IDLE, `reserved_gold == 0` for all players, and `AuctionSettled` Bevy Message is NOT fired. | BLOCKING |
| AU19-a | **GIVEN** the Auction System is in RESOLVING, **WHEN** `AbortAuction` is received, **THEN** `AbortAuction` is a no-op — **RESOLVING is uninterruptible** (see Edge Cases). Resolution completes normally: `spend_reserved_gold` fires, `S2CAuctionSettled` is broadcast, and the system transitions to IDLE. Test: assert that `AuctionSettled` IS fired (i.e., the abort does not suppress settlement) and that post-resolution economy state is correct (gold deducted, card added). *(This state is effectively unreachable under correct Bevy scheduling — test as a defensive regression guard.)* | BLOCKING |
...
| M7-a | **GIVEN** an accepted bid is processed, **WHEN** the Economy System fires `S2CGoldBroadcast` for the bidding player, **THEN** the broadcast payload includes both `gold: u32` and `reserved_gold: u32` for the affected player, so the receiving client can compute `free_gold = gold - reserved_gold`. *`S2CGoldBroadcast` includes `reserved_gold` per NP R3 (2026-04-30). ~~Sprint prerequisite~~ **RESOLVED** — no cross-team blocker.* | BLOCKING |
...
| OQ7 | **Late-game wealth disparity — ⚠️ Pillar Risk: Acknowledged.** At 35g vs 10g, the wealthy player can open at `opponent_gold + 1g` and win without a real contest. This is a known violation of "No idle spectating" for the economically trailing player. **Decision deadline: before M2 implementation begins.** *(Moved forward from "before M3 closes" — a structural fix such as a bid ceiling relative to opponent gold would require network protocol schema changes and economy GDD revisions; discovering this at M3 is expensive.)* Monitor gold differential during M2 playtesting — if gap regularly exceeds 20g before Round 9, escalate to a design sprint before M2 closes. **Current mitigation candidates are all structurally flawed:** (1) trailing-player interest multiplier — does not close the gap at auction time, adds degenerate "hold less gold" incentive; (2) free-bid floor — directly breaks the lie-detector invariant (a bid placed for free is not a real signal); (3) scaled gold consolation on loss — farmable (open at minimum on cards you don't want, force opponent to spend, collect consolation). A modified (3) with multi-bid minimum and high threshold (20g+ deficit) is closest to viable but requires further design work. **Prioritize a structural constraint** (e.g., max single bid capped at opponent's free gold) that preserves the information symmetry the lie-detector mechanic requires. | Game Designer | Before M2 implementation begins |
