...
| 13 | Board Rendering | `gdd/board-rendering.md` | Designed | All 8 sections + Visual/Audio + UI Requirements + Acceptance Criteria (25 ACs, 19 BLOCKING). 5 OQs: Sang Méprise signal, camera setup, ResolutionEvent enum, spawn range signal, unit atlas dimensions. /design-review pending (fresh session). |
| 14 | Hand UI | `gdd/hand-ui.md` | Designed | All 8 sections + Visual/Audio + UI Requirements + Open Questions. 24 ACs (22 BLOCKING, 2 ADVISORY). Drag-and-stage PLACEMENT flow, DRAFT_INITIAL 3×3 grid, Instant card fan plate, 5-second urgency threshold. 5 OQs: reserve mana extensibility, zoom→click activation, Ready signal ownership, GhostPlacementChanged interface, card asset pipeline. /design-review pending (fresh session). |
| 15 | Shop / Auction UI | `gdd/shop-auction-ui.md` | Designed | All sections complete + Visual/Audio + UI Requirements + Open Questions. 1 new formula (local_free_gold). 5 OQs: bid text input, tooltip persistence, S2CGoldBroadcast reserved_gold (NP update needed), screen layout split, C2SSignalReady NP registration. /design-review pending (fresh session). |
