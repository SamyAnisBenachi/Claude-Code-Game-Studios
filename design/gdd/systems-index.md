...
| 18 | Prism System | `gdd/prism-system.md` | Designed | /design-review pending (fresh session). All 8 sections + Visual/Audio + UI Requirements + Open Questions. 22 ACs (17 BLOCKING, 5 ADVISORY). 5 OQs: hand API unification (OQ1), S2CCardAcquired registration (OQ2), PrismReward enum removal (OQ3), server-rng conditional note (OQ4), prism_strike self-target (OQ5). Registry: 2 items (prism_strike, prism_reserve), 2 constants (prism_strike_damage, prism_strike_mana_cost), 1 network_message (S2CCardAcquired). |
| 19 | Class System | `gdd/class-system.md` | Needs Revision | All 8 sections + UI Requirements + Open Questions complete (27 ACs, 26 BLOCKING). 7 tokens registered (Mummy, Sinistro, Chacha Noir, Seed, Madoll, La Gonflable, La Sacrifiée). 8 formulas + 5 constants registered. 4 OQs: Xelorium timing, Sang Méprise reconnect gap (NP), Rollback+HASTE, Madoll spell-cost scope. /design-review pending (fresh session). |
