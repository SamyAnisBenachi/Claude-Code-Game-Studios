...
| 13 | Board Rendering | `gdd/board-rendering.md` | In Review | /design-review R4 2026-04-30: NEEDS REVISION → revised in-session. 7 blockers resolved: OQ-BR-06 RESOLVED (C2SRequestSnapshot defined in NP-43; stale text removed from 3 locations; BR-18c/BR-EC-STUCK ungated); SpriteAlphaLens API corrected (`set_alpha()` → `with_alpha()`); Player Fantasy absolute ceiling claim removed (no single absolute ceiling); draw call worst-case corrected (cell nodes = 6 batches, status icons = 3–5, true worst case ~18–25); F4 5-destroy ceiling corrected (18,000ms not 14,600ms); 2 missing ACs added (BR-EC-PLACEMENT-STUCK, BR-CAMERA-PROJ); Rule 14 priority tiers added (Tier 1: SHIELDED/TAUNT/STEALTH/IMMUNE always visible); NP-43 rate-limit documented in all 3 recovery paths; OQ-BR-10 RESOLVED (Approach A confirmed). Re-review recommended (R5). |
| 14 | Hand UI | `gdd/hand-ui.md` | Needs Revision | /design-review R1 2026-04-30: NEEDS REVISION → revised in-session (4 P0 + 6 P1 resolved). /design-review R2 2026-04-30 (fresh session, full panel): 6 new blockers resolved — Rule 9 rewritten (200ms grace window; preserves Pillar 1); Rule 13 auto-decrement removed ([ + ] blocks at ceiling); sold-out ambiguity accepted (all non-arrival → timeout → revert); stale hand state documented; Rule 5c + HU-28/HU-28b updated with S2CActivationRejected (OQ8: NP gate); HU-26 rewritten ([ + ] path trigger). Rule 5d added (DRAFT_SHOP drag suppressed). VA-10 added (PASSIVE_LOCKED indicator). HU-24 extended (drag sprite reset). Re-review R3 recommended after OQ8 resolved in NP GDD. |
...
| 19 | Class System | `gdd/class-system.md` | Approved | /design-review pass 2 2026-04-30: APPROVED (30 blockers resolved in-session). CS-12 Cra RANGE anchor added; Rollback↔Garde-Temps strategic tradeoff documented; garde_temps_used_this_game ownership declared (Game Session System); CS-10 Craps division guard; CS-11 naming standardized; Miranda adjacency bound; Madoll Spell-type-only; OQ-CS-2 closed; NP-6/7/8 registered (Sinistro, Miranda, Chacha Noir protocol gaps); 6 UI requirements added (class picker state, Xelorium drain, Sinistro display, Garde-Temps visual); PIERCE as Sadida AR counter. Shava Shavien tails-to-opponent kept by design decision. |
| 20 | Card Animations | `gdd/card-animations.md` | In Review (Pass 3 revised) | /design-review Pass 3 2026-04-30: NEEDS REVISION → revised in-session (15 blockers resolved across 7 specialist domains). Key fixes Pass 3: ghost events removed (ResolutionGroupReady no upstream emitter; FogLiftReady fog system removed); DamageNumberSpawnRequested added to Rule C-10 (Board Rendering emitter, payload defined); GroupDrainedSignal introduced as sole CA emission for GAME_OVER path; CA-21 rewritten (BoardCell was PlayTarget enum, not component — replaced with LaneCell + BoardLayout.cell_to_world()); CA-24 rewritten (2 entities Playing, not 1 — replacement tween keeps A active); PlacementPhaseAnimator marker component defined; StagedObjectiveRevealQueue added to plugin build spec; queue-during-queue 4 cleanup steps enumerated; F2 ranges unified 400–549 ms, assert changed to strict `<`; sub-step floor corrected to 451 ms; V.1 hierarchy rewritten as visual weight not duration; settlement overlay D4 "affective minimum" loophole replaced with D1 justification; hover cancel 40 ms floor added; optimistic rollback reversal spec (Crimson Slate flash); fog_lift_duration_ms references removed; 4 CA fields added to game-config.md; CA-3 pre-impl gate; CA-25 added (DamageNumberSpawnRequested spawn trigger); OQ-CA-13/14 added; ordering invariant corrected. 25 ACs (21 BLOCKING, 4 ADVISORY). 14 OQs. Re-review recommended in fresh session. |
