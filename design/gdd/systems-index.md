...
| 7 | Network Protocol | `gdd/network-protocol.md` | In Review | /design-review R3 2026-04-30: MAJOR REVISION NEEDED → revised in-session (17 blockers resolved). Key fixes: ResolutionEvent 10 new variants (CombatDamage, KeywordTriggered, DisplacementEvent, SubStepMarker, FinalBlowFired, AppearanceFired, DeathTriggerFired, EndOfTurnFired, SpawnRangeChanged + enums DamageKind/DisplacementKind/KeywordPayload); BoardSnapshot added seeds + sinistros; PlayerSnapshot added class_id; UnitBoardState 7 keyword state fields + max_hp; StructureBoardState max_hp; GoldAwardReason::PrismReward removed; AcquisitionSource rename + FreeCardPick added; S2COpponentSubmitted added; AuctionSnapshot starting_price added; NP-23 BLOCKING; NP-19 rewritten; NP-25/26 rewritten; NP-7/12 rewritten; integration annotations NP-13/17/24/28/29; NP-30-35 new ACs; OQ5 closed. Re-review recommended (fresh session, lean mode). |
...
| 11 | Auction System | `gdd/auction-system.md` | Needs Revision | **Signature mechanic** — /design-review 2026-04-30 (pass 3): NEEDS REVISION → revised in-session (14 blockers resolved). Key fixes: AuctionSnapshot `starting_price` added + `last_accepted_bid` sentinel fixed to 0 (NP wins over GDD); Rule 5 hardcoded 5000ms → `GameConfig.auction_timer_reset_seconds * 1000`; tick_delta_ms clamp (1000ms) added to Rule 6 (lag-spike premature resolution); `saturating_sub` + `debug_assert` in `spend_reserved_gold`; `saturating_sub` in bid validation; AU1-b fallback replaced with ADR-008 integration test gate; RESOLVING-is-uninterruptible specified (AbortAuction during RESOLVING is no-op); M7-c added (outbid player S2CGoldBroadcast); AU8 split into AU8 (Unit) + AU8-pool (Integration); AU20 reformulated as before/after assertion; bid button total-commitment label required; timer ease-out fixed-snapshot clarified; "Raise bid" / leading-state three-way inconsistency resolved (buttons hidden, not disabled); AU19 split into AU19-a + AU19-b; M7-a stale prerequisite removed; OQ7 deadline moved to **before M2 implementation begins** (mitigation candidates found structurally flawed). Re-review recommended in fresh session. |
...
| 16 | HUD | `gdd/hud.md` | In Review | /design-review 2026-04-30: MAJOR REVISION → revised in-session. Layout restructured: both gold zones consolidated to top-right (diagonal gold comparison was structural failure of peripheral omniscience fantasy); mana moved to bottom-left. Rule 10 vs Rule 13 GAME_OVER reconnect tiebreak resolved (snapshot wins, then re-freeze). round_counter_visible (was round_suffix_visible); round counter remains visible at GAME_OVER. Stone-thud silenced during RESOLUTION. lane-0 panic path guarded. [[bool;5];2] reclassified as hard dep on lane_count. timer_duration_ms → compiler constraint. 7 new ACs (HUD-22–28): round counter format, GAME_OVER visibility, HUD root LOBBY, cold-start placeholder, idempotency, FROZEN+snapshot, sub-label entities. 4 AC field name errors fixed (HUD-03/08/17/20). OQ-HUD-01 reclassified NP-blocking; OQ-HUD-02 design-rejected. NP GDD amendment required: reserved_gold missing from PlayerSnapshot. 29 ACs (25 BLOCKING, 4 ADVISORY). UX spec needed: /ux-design hud. Re-review recommended in fresh session. |
