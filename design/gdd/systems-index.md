...
| 6 | Round State Machine | `gdd/round-state-machine.md` | Approved | /design-review R2 full 2026-05-01: APPROVED (revisions accepted inline). 13 blockers resolved: Rule 13 rewritten to NP hybrid heartbeat model (was "(not a custom heartbeat message)" — contradicted NP Rule 8); disconnect_trackers type f32→u32 ms; OnConnected tracker reset specified; F2 Bevy scheduling invariants table added; Rule 14 timer_duration_ms u32→Option<u32> (cross-GDD note to NP GDD); Rule 7 Auction System IDLE invariant added; auction_max_duration_seconds default 120→300 + safe range lower bound 60→120; RSM-34/RSM-38 ADVISORY→BLOCKING; RSM-9 rewritten to RSM timing AC; RSM-11b (slot persistence), RSM-29b (trigger_index), RSM-29c (auction-followup timer), RSM-33a (R=0 guard), RSM-35b (GAME_OVER snapshot ordering) added; Rule 11 GAME_OVER ordering explicit; OQ-4 and OQ-6 resolved; auction_followup_placement_timer_seconds (default 12s) added to Tuning Knobs. Pre-impl note: NP GDD S2CPhaseChanged timer_duration_ms also needs u32→Option<u32> update. |
| 7 | Network Protocol | `gdd/network-protocol.md` | Approved | /design-review R7 lean 2026-05-01: APPROVED (revisions accepted inline, no re-review). 4 blockers resolved: S2CPrismRespawned + S2CPrismRewardDropped added to S2C table + Interactions + Downstream Dependents + NP-56/57 ACs (pending NP registration from prism-system.md); EndOfTurnFired removed from inner-sub_step invariant list (no inner field in struct); NP-52 ADVISORY→BLOCKING (Rule 4 consistency); NP-53 ADVISORY→BLOCKING (matches MUST in prose, test guidance added). Stale *(Not Started)* labels updated to *(Approved)* in Downstream Dependents. OQ-1/2/3 HIGH Lightyear verification remain as pre-implementation gates. Implementation-ready. |
...
| # | System | File | Status | Notes |
|---|---|---|---|---|
| 17 | Keyword System | `gdd/keyword-system.md` | Approved | /design-review R6 lean re-review 2026-05-01: APPROVED. 0 new blockers. 3 doc-hygiene items applied inline: KW-041 struck-through row added (D7 rationale); OQ-NP2 strikethrough formatting corrected; OQ-KS3 R6 verification note added (combat-resolution.md now Approved — verify OQ4 before COUNTERATTACK implementation). Cumulative: R1–R6 closed 86+ blockers across 6 reviews. All creative decisions settled. Implementation-ready. |
