...
**Rule 7 — Reconnect snapshot.** On any client connection (initial join or reconnect), before any other messages, the server sends `S2CGameSnapshot` containing complete authoritative game state. The client discards locally buffered messages received before snapshot processing completes and rebuilds its view from scratch. See `docs/architecture/adr-011-reconnect-snapshot.md` (ADR-011) for the full reconnect flow, mandatory send order (`S2CHandshake` → `S2CGameSnapshot` → `S2CObjectiveIdentities` → `S2CPhaseChanged`), and the `ReconnectTracker.deferred_queue` / `snapshot_sent` server-side gating that enforces TR-NP-006.
...
struct PlacedCardSubmit {  // C2S payload — used in C2SSubmitPlacement.placements
    card_id:        CardId,
    target:         PlayTarget,
    current_mana_spend: u32,  // mana drawn from current-round mana
    reserve_mana_spend: u32,  // mana drawn from reserve pool
}
...
**`C2SSubmitPlacement` server validation:** The server validates the full batch before accepting. For each entry, `current_mana_spend + reserve_mana_spend == card.cost`. Across the batch, `sum(reserve_mana_spend) <= player.reserve_mana` and `sum(current_mana_spend) <= player.current_mana`. If any card is not in the player's hand, any `lane`/`cell` value is out of range (1-5 / 1-8), any target is not legal for that card, any spawn/occupancy rule fails, or any split is invalid, the entire batch is silently discarded (Rule 4). **If any `card_id` appears more than once in the batch, the entire batch is silently discarded** — validation runs against the original hand snapshot before applying any changes; a duplicated card_id passes the "in hand" check on first pass but must be caught by explicit dedup check. Partial acceptance is not supported.
...
> 1. ~~**Per-client unicast API**~~ — **RESOLVED 2026-05-02 for Prism S2C messages.** Lightyear 0.26.4 uses `ServerMultiMessageSender`, `PeerId`, and `NetworkTarget`. Server systems send reliable targeted S2C messages with `sender.send::<Message, ReliableChannel>(&msg, server, &NetworkTarget::Single(peer_id))`. For public all-player delivery such as `S2CPrismRespawned`, use the same API with `NetworkTarget::All`; for owner-only delivery such as `S2CPrismRewardDropped`, use `NetworkTarget::Single(owner_peer_id)`. The older `ConnectionManager` / `send_message_to_target::<Channel, Message>` sketch is invalid.
...
struct PlacedCardReveal {  // element of S2CPlacementReveal.placements — S2C only; distinct from PlacedCardSubmit (C2S)
    ...
    owner_id: PlayerId,
    target:   PlayTarget,
    // current_mana_spend/reserve_mana_spend omitted: mana accounting is server-side after submit
}
...
1. ~~**Lightyear 0.26 per-client unicast API**~~ — **RESOLVED 2026-05-02.** Verified against local `lightyear_messages-0.26.4` source and `tests/evidence/lightyear-026-verification.md`: server S2C targeted sends use `ServerMultiMessageSender::send::<Message, Channel>(&msg, server, &NetworkTarget::Single(peer_id))`. Identifier type is `PeerId`, not `ClientId`; there is no `ConnectionManager::send_message_to_target` API. Prism-specific resolution: `S2CPrismRewardDropped` is owner-only via `NetworkTarget::Single(owner_peer_id)`, and `S2CPrismRespawned` is public all-player delivery via `NetworkTarget::All` on `ReliableChannel`. **Owner:** Network programmer. **Status:** Closed.
