...
| NP-50 | **GIVEN** the server is in PLACEMENT phase and a client sends `C2SActivateCard { card_id }`, **WHEN** the server processes it, **THEN** the message is silently discarded per Rule 4 — no S2C response of any kind is sent (no `S2CGoldUpdate`, no hand change, no economy side effect). | BLOCKING |
...
| NP-52 | **GIVEN** the server is in DRAFT_INITIAL or PLACEMENT phase and a client sends `C2SPlaceBid { amount }`, **WHEN** the server processes it, **THEN** the message is silently discarded per Rule 4 — no `S2CAuctionBidRejected` is sent and auction state is unchanged. | ADVISORY |
| NP-53 | **GIVEN** `S2CResolutionEvent` is emitted during RESOLUTION, **WHEN** any `TaggedEvent` in the batch contains a `ResolutionEvent` variant with an embedded inner `sub_step` field (DisplacementEvent, CombatDamage, DeathTriggerFired, KeywordTriggered, AppearanceFired, FinalBlowFired, EndOfTurnFired), **THEN** the variant's embedded `sub_step` value MUST equal `TaggedEvent.sub_step` for that entry. A mismatch is a server bug and MUST be caught by server-side assertion before the batch is dispatched. | ADVISORY |
