...
| **Shop/Auction UI** | Upstream → Card Animations | Emits `AuctionPanelTransitionRequested` (350 ms slide), `TimerBarEaseRequested` (120–150 ms ease-out + 60 ms flash), `TimerColorZoneRequested` (300 ms cross-fade), `GoldTickRequested` (≤ 150 ms tick), `SettlementOverlayRequested` (1.5 s overlay — **Decoration Test justification (D1):** the 1.5 s creates a mandatory space for the player to process the auction outcome and transition their mental state from "bidding" to "shopping" before DRAFT_SHOP begins immediately after. This is a state change the player must act on — without it, the DRAFT_SHOP timer is already running before the player has acknowledged the settlement result. D1 is satisfied: after this animation, the player has processed information they will act on. OQ-CA-07 resolved.), `NoBidsTransitionRequested` (200 ms desat + 400 ms fade — see OQ-CA-08). |
...
- **If `S2CPhaseChanged(GAME_OVER)` arrives during an `AnimGroup` execution mid-RESOLUTION:** the current `AnimGroup` completes in full (board-rendering Rule 10 — no mid-tween interrupt). Card Animations continues ticking the current group's tweens normally. When `group_timer.finished()` is detected and `PendingPhaseChange` contains `GAME_OVER`, Card Animations emits `GroupDrainedSignal` (see Rule C-8 GAME_OVER drain signal). Board Rendering's `ResolveStateMachine` receives this signal and executes the skip path: skipping remaining groups, running `ResolutionObjectiveReveal` for any buffered `ObjectiveDestroyed` events, then transitioning to `GameOver`. **Known design compromise:** at worst-case GAME_OVER arrival (t=1 ms into a 600 ms AnimGroup), the player waits up to 599 ms of animation after the game is already decided. **Note:** this wait will most commonly land on the game-deciding AnimGroup — the one containing the final ObjectiveDestruction event — since GAME_OVER arrives precisely when the last objective is destroyed. This is not a rare edge case; it is the modal case for winning and losing rounds. **Player Fantasy tension:** this is a known conflict with the Decoration Test D1 — post-verdict animation delivers no new information to the player. This compromise is accepted for the friend-game scope because completing the current group is simpler than a mid-group interrupt. A lower-complexity mitigation (advance `group_timer.elapsed` to `group_timer.duration` on GAME_OVER receipt, draining the group within the next frame) is viable — implement before any public launch if playtests surface it as felt friction.
...
| Damage number float duration | `float_tween_duration_ms` | 500 ms | 400–549 ms *(see joint constraint below)* | Numbers linger, cluttering board during subsequent sub-steps | Numbers exit before player reads damage |
| Damage number fade duration | `fade_tween_duration_ms` | 500 ms | 400–549 ms *(see joint constraint below)* | Visible numbers stack; combined despawn_delay_ms must satisfy < `resolution_sub_step_duration_ms − 50` | Abrupt flicker disappearance |
...
**OQ-CA-07 — "YOU WON" overlay duration** *(RESOLVED — 2026-04-30, D1 reframing 2026-04-30 Pass 3)*
The 1.5 s overlay duration is justified via D1: the dwell creates mandatory processing time before DRAFT_SHOP begins, which is a state change the player must act on. Reframed from "affective minimum D4 exception" to "D1 state-change communication" — the player uses this time to transition their mental state from bidding to shopping. The D4 "affective minimum" framing has been removed from this GDD to close an unfenced loophole in the Decoration Test. The Decoration Test justification is documented in the Shop/Auction UI Interactions row. No change to duration required.
...
**OQ-CA-13 — Settlement overlay audio ownership** *(Owner: audio-director + shop-auction-ui.md)*
Three decisions unresolved before the settlement story can begin: (a) who fires the audio cue — Shop/Auction UI is the likely trigger owner since it knows the win/loss outcome from `S2CAuctionSettled.winner`; (b) audio content type — win jingle (stinger, one-shot SFX) vs music swell (background music state transition) require different audio system implementations; (c) is there a duck rule or holdoff during settlement — if ambient music plays during DRAFT, does it duck for the overlay? No spec exists. The audio-director must resolve all three before the settlement story enters sprint.
