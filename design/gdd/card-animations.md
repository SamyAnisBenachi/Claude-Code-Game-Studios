...
|---|---|---|---|
| `SpriteAlphaLens` | `Sprite` | `sprite.color = sprite.color.with_alpha(value)` (f32) | Fog lift, death fade, damage-number fade-out, NO BIDS desaturation, card-draw alpha-in |
...
| Event | Behavior |
|---|---|
| Tween start | A `TweenAnim` controller is spawned by the system responding to the triggering event, in the same frame the tween begins. Single-animation shorthand may place `TweenAnim` on the target entity; multi-animation paths spawn separate controller entities with `AnimTarget::component::<T>(target)`. Never pre-insert on entity spawn. |
| Tween completion | **TweenAnim is NOT removed by Card Animations lifecycle helpers.** Use `with_destroy_on_completed(false)` for reusable/cancel-replace controllers. The next animation replaces it via `set_tweenable`. Damage-number controllers may be released when their owning damage-number entity despawns. |
| Tween cancel-replace | `tween_anim.set_tweenable(new_tween)` (validated by Story 002 against the Bevy-0.18-compatible `bevy_tweening v0.15.0` API). NEVER despawn-and-respawn target entities (loses game-state components: `LaneCell`, `UnitStats`, `LeaderTag`). NEVER write `Transform.translation` directly while a `TweenAnim` is actively mutating that field (board-rendering BR-16 invariant). |
| Entity despawn during active tween | Target-owned tween controllers are released atomically with entity despawn. Explicit controller entities targeting the despawned entity must be swept by the same rebuild/despawn path. |
| Concurrent animators on same entity | Independent `TweenAnim` controllers may target different components (`Transform`, `Sprite`, `TextColor`) and advance independently. |
| Same-component parallel animation | `Tracks<T>` is removed in the local Bevy-0.18-compatible `bevy_tweening v0.15.0` crate. If one entity needs two `Transform` animations simultaneously, spawn separate `TweenAnim` controller entities targeting the same entity via `AnimTarget::component::<Transform>(target)`. If the tweens affect different fields, use field-specific lenses (for example X-only/Y-only translation or scale-X-only). If they affect the same field, use one composite lens/tween so write order cannot overwrite state. |

**bevy_tweening compatibility note (resolved 2026-05-02):** Story 001/002 implementation evidence shows the workspace uses `TweenAnim`, `PlaybackState`, and `TweenState` from `bevy_tweening v0.15.0`, the Bevy 0.18-compatible release pinned in the workspace. Older GDD/stories may still say `Animator<T>` or `AnimatorState::Playing`; for implementation and tests, read that as `TweenAnim` with `playback_state == PlaybackState::Playing`.

**PLACEMENT-phase animator marker:** Every entity receiving an animation during the PLACEMENT phase (drag-lift, snap-back, hover, cell-highlight) must also have a `#[derive(Component)] struct PlacementPhaseAnimator;` marker component inserted alongside the `TweenAnim` controller in the same frame, either on the target entity for single-animation shorthand or on the explicit controller entity if `AnimTarget` is used. The `PlacementCancelAllAnimsRequested` handler queries `With<PlacementPhaseAnimator>` to identify which controllers/entities to cancel. The marker is NOT removed on cancel — it persists until the entity is despawned or `BoardRebuildRequested` resets the board.

**Damage-number lifecycle components:** Damage numbers spawn as world-space text entities with `Text2d`, `TextFont`, `TextColor`, `LineHeight`, `Transform`, `DamageNumber`, and `DespawnAfter(Timer)`. `DamageNumber` is a marker used by rebuild cleanup. `DespawnAfter` is defined as `#[derive(Component)] struct DespawnAfter(pub Timer);` and is initialized at spawn time from F2. The text value is `damage_value.to_string()`. The text fade tween targets `TextColor`; the float tween targets `Transform`.
...
│   Board Rendering → PlacementRevealAnimReady { entries },         │
...
**Domain events Card Animations consumes** (intra-client `#[derive(Message)]` types):

- From Board Rendering: `PlacementRevealAnimReady { entries: Vec<PlacementRevealEntry> }`, where `PlacementRevealEntry { unit: Entity, lane: u8, cell: u8 }` identifies each revealed unit; `ObjectiveDestroyedAnimReady`; `BoardRebuildRequested` (cancel all animators); **`PlacementCancelAllAnimsRequested`** (cancel all PLACEMENT animators on `S2CPhaseChanged(RESOLUTION)` — normal round path, not reconnect; see CA-21 and Edge Cases §2); **`DamageNumberSpawnRequested { target: Entity, damage_value: u32, event_id: u32 }`** (emitted by Board Rendering when a `ResolutionEvent` variant carries damage; `event_id` indexes deterministic jitter via F3)
...
|---|---|---|
| **Board Rendering** | Upstream (events in) → Card Animations | Emits `PlacementRevealAnimReady { entries: Vec<PlacementRevealEntry> }` with `PlacementRevealEntry { unit, lane, cell }`, `ObjectiveDestroyedAnimReady`, `BoardRebuildRequested`, **`PlacementCancelAllAnimsRequested`**, `DamageNumberSpawnRequested { target, damage_value, event_id }` after authoritative state updates. Populates `AnimQueue` directly (no per-group message — Card Animations polls `group_timer.finished()`). **Critical:** `BoardRebuildRequested` (on `S2CGameSnapshot`) cancels all in-flight tweens, clears all per-entity animator state, resets `AnimQueue`, and clears `StagedObjectiveRevealQueue` — no replay on reconnect. `PlacementCancelAllAnimsRequested` is emitted on `S2CPhaseChanged(RESOLUTION)`; Card Animations cancels all `PlacementPhaseAnimator`-marked entities and overwrites their `Transform.xy` via `BoardLayout.cell_to_world(lane, cell)`, preserving Z. Receives `GroupDrainedSignal` from Card Animations for GAME_OVER path. |
...
| **Server-side RNG** | NONE | No client-side randomness in animations. Any visual variation must be deterministic from event payload (e.g., damage-number jitter offset = `jitter_table[event_id % 8]`). |
...
**Why this formula is explicit:** Both tween controllers (`TweenAnim` float targeting `Transform` and `TweenAnim` fade targeting `TextColor`) run concurrently for the same damage-number entity. The entity must not be despawned until both complete — premature despawn causes mid-air blink-out; missed despawn causes an entity leak. Using `max()` accounts for any asymmetric tuning without code changes.

---

**F3 — Damage number position jitter** *(RESOLVED — 2026-05-02)*

`offset_px = jitter_table[event_id % 8]`

Provides deterministic position offset for damage numbers when two simultaneous hits target the same unit in one sub-step. Prevents number stacking at identical world positions without client-side RNG. `jitter_table` is a static 8-entry table of pixel offsets applied from the unit torso origin:
...
`jitter_table_len = 8`. The first damage number for a deterministic event stream may be unoffset (`event_id=0`), and subsequent simultaneous events spread around the torso without covering the HP bar. If more than 8 simultaneous damage events target the same unit, offsets wrap deterministically by modulo; this is acceptable because the visual case is already saturated.
...
- **If a PLACEMENT animation (snap-back, drag-lift, cell-highlight) is mid-tween when `S2CPhaseChanged(RESOLUTION)` arrives on the normal game path (not reconnect):** Board Rendering processes `S2CPhaseChanged(RESOLUTION)` first, updates `BoardRenderState`, then emits `PlacementCancelAllAnimsRequested` (see Rule C-10). Card Animations' cancellation system consumes this event in the same `App::update()` tick (same-frame delivery guaranteed by `CardAnimationsSet::React.after(BoardRenderSet::ScheduleTweens)` — see Rule C-10 SystemSet ordering note). All in-flight PLACEMENT `TweenAnim` controllers (identified by `PlacementPhaseAnimator` marker component — see Rule C-7) are cancelled and each target entity's `Transform.translation.xy` is overwritten by calling `BoardLayout.cell_to_world(lane_cell.lane, lane_cell.cell)` using the entity's `LaneCell { lane: u8, cell: u8 }` component while preserving Z. The 250 ms hard cap (Rule C-2) bounds the maximum mid-tween duration discarded (at most 250 ms of animation is lost). This is the common-case path — every round transitions PLACEMENT → RESOLUTION this way. See CA-21.
...
- **If the same unit entity receives multiple `DamageNumberSpawnRequested` events in the same `AnimGroup` (unit takes damage from two simultaneous sources in one sub-step):** each event spawns a distinct damage-number entity. No entity reuse. The entity origin position is offset by F3 (`jitter_table[event_id % 8]`; deterministic jitter from event payload — no client-side RNG) to prevent numbers stacking at identical world positions. Each entity has its own independent float and fade `TweenAnim` controllers with its own `despawn_delay_ms` computed via F2.
...
- **If `PlacementRevealAnimReady` targets a unit entity that also has an active `TweenAnim` controller mutating `Transform` from a concurrent `DisplacementAnimRequested` in the same `AnimGroup`:** the placement-reveal flip uses `SpriteColorLens` plus a scale-X transform lens. There is no Sprite/Transform component conflict. If translation and scale must animate simultaneously on the same `Transform`, use separate controller entities with `AnimTarget::component::<Transform>(unit)` and field-specific lenses, or one composite lens if two tweens need to write the same field. Do not use `Tracks<Transform>`; it is removed from the local Bevy-0.18-compatible bevy_tweening crate.
...
|---|---|---|
| Board Rendering | `gdd/board-rendering.md` | Emits `PlacementRevealAnimReady { entries: Vec<PlacementRevealEntry> }` with `PlacementRevealEntry { unit: Entity, lane: u8, cell: u8 }`, `ObjectiveDestroyedAnimReady`, `BoardRebuildRequested`, `PlacementCancelAllAnimsRequested`, `DamageNumberSpawnRequested { target: Entity, damage_value: u32, event_id: u32 }`. Card Animations drives all RESOLUTION tweens from `AnimQueue` populated directly by Board Rendering (Rule 9 of board-rendering.md). Owns `AnimGroup`/`AnimQueue` data schema. Receives `GroupDrainedSignal` from Card Animations for GAME_OVER path. |
...
| bevy_tweening | `Cargo.toml` (version 0.18-compatible) | Core tween library. The workspace currently resolves `bevy_tweening v0.15.0` for Bevy 0.18, exposing `TweenAnim`, `AnimTarget`, `PlaybackState`, `TweenState`, `Tween`, and `Sequence`. `Tracks<T>` is removed; parallel same-component animation uses independent `TweenAnim` controllers plus `AnimTarget::component`. Custom lenses extend it via the `Lens<T>` trait. |
...
- Settlement overlay: fire audio at `+0 ms` from `SettlementOverlayRequested` receipt. **Audio ownership required:** the 400 ms settlement overlay is the most emotionally weighted moment in the draft loop. Audio content (win jingle, loss sting, or neutral music swell) is unspecified — the audio-director must assign ownership before the settlement story is implemented.
...
| CA-3 | **GIVEN** `PlacementRevealAnimReady { entries }` is received with 5 `PlacementRevealEntry { unit, lane, cell }` entries, **WHEN** one `App::update()` tick runs, **THEN** all 5 unit entities have reveal `TweenAnim` controllers with `playback_state == PlaybackState::Playing`. Verified by asserting `Playing` state on all 5 reveal controllers post-update. Implementation note: the system must iterate all 5 entries in a single pass before any `apply_deferred` flush separates them; no `apply_deferred` may be injected mid-system by any registered plugin. Scheduling invariant must be validated via system-order inspection during code review in addition to this test. | BLOCKING |
...
| CA-15 | **GIVEN** a unit entity needs simultaneous REPEL displacement (`Tween<Transform>`) and placement-reveal flip (`SpriteColorLens` + `TransformScaleXLens`), **WHEN** both tweens are spawned, **THEN** the relevant `TweenAnim` controllers exist simultaneously and have `playback_state == PlaybackState::Playing` after one tick. | BLOCKING |
...
| CA-18 | **GIVEN** a unit entity at `Transform::IDENTITY` requires two simultaneous `Tween<Transform>` animations — tween A moving X from 0.0 to 100.0 over 600 ms and tween B moving Y from 0.0 to 60.0 over 600 ms — implemented as two `TweenAnim` controller entities targeting the unit via `AnimTarget::component::<Transform>(target)`, using X-only and Y-only translation lenses, **WHEN** `Time<Virtual>` is advanced by 16 ms and `App::update()` runs, **THEN**: (a) exactly two `TweenAnim` controllers target the unit's `Transform` and both have `playback_state == PlaybackState::Playing`, AND (b) the entity's `Transform.translation.x` is in the range (0.0, 100.0) exclusive, AND (c) the entity's `Transform.translation.y` is in the range (0.0, 60.0) exclusive. The range assertions prove both tweens are advancing simultaneously without prescribing an exact easing-curve value. | BLOCKING |
...
| CA-21 | **GIVEN** a PLACEMENT-phase animation (snap-back, drag-lift, or cell-highlight) is in `PlaybackState::Playing` on an entity/controller that carries a `PlacementPhaseAnimator` marker and the target board entity carries `LaneCell { lane: u8, cell: u8 }` (see Rule C-7), **WHEN** `PlacementCancelAllAnimsRequested` is processed (emitted by Board Rendering on `S2CPhaseChanged(RESOLUTION)`, delivered same-frame via SystemSet ordering), **THEN**: (a) all `PlacementPhaseAnimator` controllers are no longer in `Playing` state after `App::update()`, verified via `Query<&TweenAnim, With<PlacementPhaseAnimator>>`, AND (b) `world.get::<Transform>(entity).unwrap().translation.truncate() == board_layout.cell_to_world(lane_cell.lane, lane_cell.cell)` where `board_layout` is read from `Res<BoardLayout>` and the entity's Z is preserved. | BLOCKING |
...
| CA-25 | **GIVEN** `DamageNumberSpawnRequested { target: entity, damage_value: 15, event_id: 0 }` is written via `MessageWriter<DamageNumberSpawnRequested>`, **WHEN** `App::update()` runs, **THEN**: (a) exactly one entity with `DamageNumber` marker component exists in the world, AND (b) the entity uses the world-space text layout `Text2d` + `TextFont` + `TextColor` + `LineHeight` + `Transform`, AND (c) float and fade `TweenAnim` controllers are in `PlaybackState::Playing`, AND (d) the entity carries a `DespawnAfter(Timer)` component initialized from F2 (`max(float_tween_duration_ms, fade_tween_duration_ms)`). F3 applies `jitter_table[event_id % 8]` to the spawn origin. | BLOCKING |

## Open Questions

**OQ-CA-01 — `Animator::set_tweenable()` API and `AnimatorState` enum in bevy_tweening 0.18** *(RESOLVED — 2026-05-02)*
Story 001/002 implementation evidence confirms the workspace resolves `bevy_tweening v0.15.0`, the Bevy 0.18-compatible crate version currently in use. The runtime controller is `TweenAnim`, not `Animator<T>`. The public inspectable playing flag is `TweenAnim.playback_state == PlaybackState::Playing`; completion is inspected via `TweenState`. `TweenAnim::set_tweenable(...)` exists and is the cancel-replace path used by Story 002. ACs and stories that still say `AnimatorState::Playing` must implement/test against `PlaybackState::Playing`.

**OQ-CA-02 — `Tracks<T>` API in bevy_tweening 0.18** *(RESOLVED — 2026-05-02)*
`Tracks<T>` is removed from the local Bevy-0.18-compatible `bevy_tweening v0.15.0` crate. The crate documentation and example path replace it with independently enqueued `TweenAnim` controllers, typically as separate controller entities with `AnimTarget::component::<Transform>(target)`. For same-`Transform` parallelism, use field-specific lenses or one composite lens to avoid write-order overwrites. Story 003 is unblocked by this design change; it must not use `Tracks<T>`.
...
**OQ-CA-04 — `TextColor` as a lens target** *(RESOLVED — 2026-05-02)*
Story 001 implementation evidence confirms `TextColor` satisfies the `Lens<T>` target path: `TextColorLens` compiled, constructed a `Tween`, inserted it in a `TweenAnim`, and lerped a `TextColor` component in `tests/unit/card-animations/plugin_scaffold_test.rs`. Damage-number fade uses `TextColor`; no sprite-overlay fallback is needed.
...
**OQ-CA-11 — Jitter table definition for simultaneous damage numbers** *(RESOLVED — 2026-05-02)*
F3 now defines `jitter_table_len = 8` and an explicit static `Vec2` offset table. `DamageNumberSpawnRequested { target, damage_value, event_id }` owns the deterministic index input. Damage-number spawn stories use `jitter_table[event_id % 8]`; no client-side RNG is permitted.
