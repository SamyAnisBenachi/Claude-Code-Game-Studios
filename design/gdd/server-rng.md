...
| RNG13 | **GIVEN** session A ends (`ServerRng` resource removed) and session B starts (`ServerRng` re-inserted), **WHEN** `next_seed()` is called once in session B, **THEN** `audit_log()[0].event_type == "session_init"`, `audit_log()[0].seed_index == 0`, and `audit_log()[1].seed_index == 1` (counter resets after the sentinel, not from session A). | BLOCKING |
