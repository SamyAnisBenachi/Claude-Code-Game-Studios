...
> **Status**: Needs Revision — R2 full review 2026-04-30: NEEDS REVISION (31 blockers addressed inline). Replication Contract synced to NP R6: STUN→stunned_until_round: Option<u32>, SILENCE→Option<u32>, HASTE row added, OQ-NP1/4/5 RESOLVED. KeywordKind→GrantedKeyword. INJURED derivation fixed (UnitStats.hp + cached max_hp). LEADER stacking rule defined (no stack, earliest-placed). INJURED exhaustive grant list defined (4 variants). SILENCE design intent added to Player Fantasy. SILENCE VFX + COUNTERATTACK tooltip added to UI Requirements. SILENCE-on-WALL worked example + LEADER stacking edge case added. OUTNUMBERED Fields excluded (confirmed). Formula 2 lane-local precondition added. Formula 3 Fields note closed. Tuning Knobs: REPEL X≥1 rule, STUN duration_rounds=1 invariant, SILENCE OQ-KS-new. OQ-KS-new added (SILENCE structured duration). KW-035a assertion target fixed. KW-041 reclassified BLOCKING. 16 new ACs (KW-042–KW-057). Re-review recommended (R3).
...
**LEADER** — grants a stat bonus to all friendly units of the same family (bonus type and value per card). Snapshotted at RESOLUTION entry; persists even if LEADER dies in SS4. Recalculated fresh each round. A SILENCEd LEADER does not grant its bonus. The LEADER counts as a family member for "X [family] in play" effects. **LEADER stacking rule:** If two LEADER units of the same family are both alive at RESOLUTION entry, only the one placed earlier in the current session grants its bonus; the second LEADER's bonus is suppressed (bonuses do not stack). The earlier-placed LEADER is always deterministic from placement order. Two LEADERs of different families each grant their own family's bonus independently.

**OUTNUMBERED** — this unit's OUTNUMBERED bonus is active when the controlling player has fewer units on the board (all lanes, Minions + Structures, excluding Traps and Fields) than the opponent. Evaluated at the start of each sub-step. Can activate or deactivate mid-RESOLUTION as deaths change counts. **Board count definition:** Traps are face-down and excluded. Fields are passive lane-wide effects with no HP and are excluded (same rationale as Traps — only entities that fight count). Maximum per player: 10 (5 lanes × 1 Minion + 5 lanes × 1 Structure cap — verify Structure-slot cap against `board-lane-system.md`; confirmed max = 10 for 1v1).
...
| **SILENCE** | `UnitBoardState.silenced_until_round: Option<u32>` (NP R6 authoritative type — was incorrectly `Option<u8>` in prior versions of this table. `u32` matches `round_number` type. See OQ-KS-new for the structured duration field requirement.); `KeywordTriggered { payload: KeywordPayload::SilenceApplied { duration_rounds: u8 }, sub_step }` event on application. | Per-card structured `silence_duration_rounds` field (see OQ-KS-new — this field must exist in `cards.json` before SILENCE can be implemented). | From snapshot — `Some(R)` where R = round in which silence expires; client renders SILENCE outline while `current_round <= silenced_until_round`. |
...
| Server-side RNG | RESOLUTION RNG chain | RANGE equidistant selection, TELEPORT random-destination, Strich lane selection | OQ-KS1 (open): THREE distinct seed slots required — `range_equidistant_select`, `teleport_random_dest`, `strich_change_lane_select`. The original "one seed slot covers both" claim was incorrect. Only `range_equidistant_select` is registered in server-rng.md (via combat-resolution.md OQ3 resolution); `teleport_random_dest` and `strich_change_lane_select` still need registration before TELEPORT and Strich implementation. |
...
**Output Range:** [1, 8]. `effective_pull = 0` if caster and target already share a cell (no movement). Destination is always between `target_cell` and `caster_cell` inclusive — the target cannot overshoot the caster. **`sign(0)` note:** when `caster_cell == target_cell`, `effective_pull = min(X, 0) = 0`, so `sign(0)` does not affect the output regardless of convention used (0 × anything = 0). The implementation may use `i8::signum()` safely.
...
No interactive elements. No modal dialogs. Keywords are intended to be self-describing from their card text with one explicit exception:

**COUNTERATTACK tooltip (exception):** COUNTERATTACK's proximity rule (same-cell OR collision-halted adjacent, but NOT RANGE attackers) cannot be derived from card text alone. Per R1 user adjudication: ship a **first-encounter tooltip** for COUNTERATTACK. Tooltip copy: *"COUNTERATTACK: when this unit takes damage from a melee attacker (same cell or stopped adjacent from collision), it retaliates immediately. Does not trigger on RANGE attacks."* Surface in Hand UI (hover on card with COUNTERATTACK in hand) and Board Rendering (hover on unit with COUNTERATTACK on board). Card text remains short (`COUNTERATTACK` only); the tooltip does the explanatory work. This is the ONLY keyword with a first-encounter tooltip.
...
| KW-041 | GIVEN Player A uses ATTRACT to pull a Player B unit to Player A's Cell 1, WHEN SS6 objective damage resolves, THEN Player A's objective HP decreases by the Player B unit's ATK value (post-modifier) — ATTRACT does not grant immunity from backfire positioning. **Design note:** this is intentional skill-expression, not a gotcha — card text for high-range ATTRACT must be evaluated carefully before use. | BLOCKING |
...
| KW-046 | GIVEN a LEADER unit with HASTE is placed in SS1 of round R, WHEN the LEADER bonus snapshot is computed at RESOLUTION entry for round R, THEN no bonus is applied to eligible family units this round — the LEADER placed in SS1 was not on the board at RESOLUTION entry; eligible family units receive the bonus from round R+1 onward if the LEADER survives. | BLOCKING |
...
| KW-054 | GIVEN Player A has 2 units on board and Player B has 4 units on board at SS3 entry (Player A is OUTNUMBERED); Player A's FIRST STRIKE units kill 3 opponent units in SS3 (opponent count drops to 1); WHEN SS5 is evaluated, THEN OUTNUMBERED for Player A is false — Player A now has 2 units and opponent has 1 (2 > 1, not outnumbered); the bonus is inactive in SS5 and SS6. | BLOCKING |
...
| OQ-KS-new | **SILENCE structured duration field** — the server computes `silenced_until_round = current_round + N` but `effect_text` is a display string with no machine-readable duration. A structured `silence_duration_rounds: u8` (or equivalent) field must be added to the `cards.json` schema and to `card-data-pool.md` Detailed Rules before SILENCE can be implemented. **Priority: HIGH — blocks SILENCE implementation.** | Card Data & Pool GDD | Add `silence_duration_rounds: u8` field to `cards.json` keyword schema in `card-data-pool.md`. Update this OQ once the field is added. |
