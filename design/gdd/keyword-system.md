...
| KW-072 | GIVEN Player A unit at Cell 1 is REPELled 6 cells (maximum), WHEN `repel_destination` resolves, THEN `clamp(1 + (−1)×6, 1, 8) = clamp(−5, 1, 8) = 1`; unit does not move; zero cells are traversed (no Trap triggers). | BLOCKING |
