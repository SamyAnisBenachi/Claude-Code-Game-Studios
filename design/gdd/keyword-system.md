...
| **LEADER bonus** | `UnitBoardState.leader_bonus_atk: u8` and `leader_bonus_hp: u8` on every eligible family unit (including the LEADER itself), written after SS1 completes (post-SS1, pre-SS2); `KeywordTriggered { keyword: LeaderSnapshotTaken, leader_unit_id }` emitted at SS1-end snapshot for client tint application. `UnitBoardState.max_hp` in `S2CGameSnapshot` reflects LEADER-buffed HP — the snapshot is generated after the LEADER bonus is applied. | Persists for entire RESOLUTION even if LEADER dies in SS4; recalculated next round | From snapshot (the buffed stats reflect the snapshot; on reconnect the tint is reapplied to all units whose `leader_bonus_atk > 0`) |
...
**STUN + COUNTERATTACK:** A STUNned unit that receives melee damage does NOT fire COUNTERATTACK. STUN suppresses all keyword hooks including reactive triggers. This ruling is design decision D4 (R3). (Tracked as OQ-KS6 — resolved.)

**FIRST STRIKE + WALL:** A unit with FIRST STRIKE that is co-located with an enemy WALL at SS3 entry fires its FIRST STRIKE attack against the WALL in SS3. WALL deals 0 damage in response. If the FIRST STRIKE attack kills the WALL (reduces HP to 0), the WALL is removed in SS4 and no longer blocks movement in SS5 — the lane is now open. This is intentional counter-play: FIRST STRIKE + CHARGE X is a valid counter to WALL lane anchors.

**RANGE + WALL:** A WALL unit is a valid RANGE target. If the WALL is the nearest enemy within range, the RANGE unit attacks the WALL in SS6 (or SS3 if FIRST STRIKE). WALL's movement-halt behavior applies to advancing units, not to RANGE targeting. A RANGE unit may fire at cells past a WALL only if no enemy (including the WALL) occupies any cell between the RANGE unit and the target. In practice, if a WALL is present between the RANGE unit and its intended target, the RANGE unit fires at the WALL (the nearest enemy), not past it.

**ATTRACT collision rule for enemy targets:** Opposing units (from different players) can never occupy the same cell — when they make contact they are always 1 cell apart. An enemy unit pulled by ATTRACT stops 1 cell short of the caster's cell. This means a unit can never be pulled to the caster's own objective cell via ATTRACT. For friendly targets (same player), co-occupation is allowed and the target may stop at the caster's cell. TELEPORT is the only effect that bypasses the 1-cell-apart rule. *(OQ-KS10: board-lane-system.md must formally define this collision rule as a primary board rule.)*
