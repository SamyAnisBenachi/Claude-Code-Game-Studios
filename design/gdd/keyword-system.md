...
> **Status**: Approved — R5 lean re-review 2026-05-01: APPROVED. 1 blocker (SILENCE+HASTE edge case + KW-082) + 3 recommended items applied inline. All prior blocking items resolved across R1–R5.
...
**SILENCE as the totalizing counter:** SILENCE strips all keywords simultaneously — including WALL's blocking behavior, BODYGUARD's protection, LEADER's bonus, and all trigger hooks. This all-or-nothing scope is intentional design, not a documentation gap. SILENCE is the game's "I read them perfectly" payoff: the player who plays SILENCE on a WALL+BODYGUARD+LEADER cluster has read the opponent's entire clockwork and applied the one card that dismantles it. The keyword system's clockwork-authorship fantasy includes a wrench. This only feels fair if the receiving player could have anticipated it — anticipation must be possible from board state alone (the SILENCE card exists in the card pool; a BODYGUARD protecting the LEADER is the textbook counter-play). The design test: a player who has internalized the card pool and reads board pressure signals should be able to anticipate SILENCE risk. Early sessions where a player doesn't yet know SILENCE exists represent the learning curve, not a design failure — the high-visibility SILENCE application animation is the "now you know" moment. The bar is: after seeing SILENCE used against them once, can a player recognize and plan around the risk? A player who loses a second time to SILENCE and says "I still couldn't have known" means the design has failed. A player who loses and says "I should have protected that LEADER" has experienced the design correctly.
...
**DEATH** — fires when a unit is removed from the board in sub-step 4. Chains are sequential: if A's DEATH trigger kills B, B's DEATH trigger fires after A's completes. Simultaneous deaths fire in lane order (Lane 1–5). Kill gold (+1) is awarded to the player whose attack dealt the final damage. **DEATH trigger kill gold:** if a DEATH trigger *effect* (not a direct attack) deals the killing blow, kill gold is awarded to the triggering unit's controller — the triggering player is credited as the "attacker" for kill-gold purposes in trigger chains.
...
**STUN** — unit cannot act this round: SS2 (CHARGE X), SS3 (FIRST STRIKE), SS5 (standard movement), SS6 (attacks) are all suppressed. STUN suppresses reactive keyword hooks that require the unit to actively participate: a STUNned unit does NOT fire COUNTERATTACK when attacked, and does NOT fire other action-based trigger hooks. **STUN does NOT suppress DEATH** — a STUNned unit that is killed fires its DEATH trigger normally in SS4; dying is not a unit action. Unit remains on board and takes incoming damage normally. Lasts current RESOLUTION only.
...
>
> **Snapshot timing:** `S2CGameSnapshot` always reflects authoritative server state at the moment it is generated — not the state at RESOLUTION start. A client reconnecting mid-RESOLUTION receives a snapshot representing the current board state through the last completed sub-step. All "from snapshot" entries in this table depend on this invariant.
...
| **SILENCE** | `UnitBoardState.silenced_until_round: Option<u32>` (NP R6 authoritative type — was incorrectly `Option<u8>` in prior versions). **Server computes:** `silenced_until_round = current_round + silence_duration_rounds - 1` (expiry-inclusive). For N=1: `silenced_until_round = current_round`. **Client renders SILENCE while:** `current_round <= silenced_until_round`. Worked example for N=1: applied in round 5 → `silenced_until_round = 5`; in round 5 `5 <= 5 = true` (active); in round 6 `6 <= 5 = false` (expired) ✓ exactly 1 RESOLUTION. See OQ-KS-new for the `silence_duration_rounds` structured field requirement. **OQ-KS7 RESOLVED:** `KeywordPayload::SilenceApplied` payload is `{ duration_rounds: u8, stripped_keywords: Vec<KeywordKind> }` where `KeywordKind` is the same enum used in the `keywords` array of `cards.json`. The `stripped_keywords` list includes all runtime-active keywords stripped at the moment SILENCE applied (including INJURED-granted keywords active at application time). The animation system uses this list to play the glyph-dissolve sequence for all stripped keywords simultaneously. **NP GDD must be updated** to add `stripped_keywords: Vec<KeywordKind>` to the `SilenceApplied` variant of `KeywordPayload`. **On receipt of `SilenceApplied`:** client MUST clear all runtime INJURED-granted keyword state for that unit (including bonuses tracked from prior `InjuredBonusActive` events). SILENCE is full-RESOLUTION scoped — it does not expire mid-sub-step. | Per-card structured `silence_duration_rounds` field (see OQ-KS-new — this field must exist in `cards.json` before SILENCE can be implemented). | From snapshot — `Some(R)` where R = round silence expires; client renders SILENCE while `current_round <= silenced_until_round`. |
...
**OQ-KS8 RESOLVED:** `CounterattackFired` payload is extended to `{ target_id: EntityId }`. For multi-attacker COUNTERATTACK (unit X retaliates against attackers A and B simultaneously), the server emits one `CounterattackFired { target_id: A }` and one `CounterattackFired { target_id: B }` — one event per retaliation target. The animation system uses `target_id` to pair each COUNTERATTACK retaliation with its intended target. **NP GDD must be updated** to add `target_id: EntityId` to the `CounterattackFired` variant of `KeywordPayload`.
...
**Precondition:** Caster and target must be in the same lane. Cross-lane REPEL is not supported by this formula. Cross-lane repositioning requires TELEPORT with card text specifying the destination lane. Lane-locality is enforced by the server before the formula runs, not by the formula itself.
...
**Enemy targets** (target.owner ≠ caster.owner — 1-cell-apart collision rule applies):
`effective_pull = min(X, max(0, |caster_cell − target_cell| − 1))`

**Friendly targets** (target.owner = caster.owner — co-occupation allowed):
`effective_pull = min(X, |caster_cell − target_cell|)`

`attract_destination = target_cell + sign(caster_cell − target_cell) × effective_pull`

The server branches on target ownership before computing `effective_pull`. The enemy form ensures the target stops 1 cell short of the caster's cell; the friendly form allows co-location.
...
**Collision rule for enemy targets:** the enemy-target `effective_pull = min(X, max(0, |dist| − 1))` branch guarantees `attract_destination ≠ caster_cell` for all enemy targets — the formula handles the 1-cell-apart rule directly; no post-formula server correction is needed. For friendly targets, the unit can stop at the caster's cell (co-occupation allowed). TELEPORT is the only displacement that bypasses the 1-cell-apart rule (co-occupation explicitly allowed for TELEPORT).
...
**Server update contract:** The `outnumbered_cache` is owned by the sub-step scheduler. It is recomputed **once per sub-step boundary**: after all effects of sub-step N fully resolve (including all DEATH trigger chains in SS4), before sub-step N+1 begins. At RESOLUTION start (before SS1), the cache is initialized from the current board state — the same count that would be computed "after SS0 completes." The cache is a single atomic read from the authoritative alive-unit count; it is not updated incrementally during a DEATH chain. The sub-step scheduler is the single responsible system; no other system updates `outnumbered_cache`.
...
**STUN + COUNTERATTACK:** A STUNned unit does NOT fire COUNTERATTACK when attacked. STUN suppresses reactive keyword hooks that require the unit to actively participate — COUNTERATTACK and other action-based trigger hooks are suppressed. The STUNned unit takes incoming damage normally but produces no attack retaliation.

**STUN does NOT suppress DEATH trigger:** If a STUNned unit is killed (HP reduced to 0 in SS3 or SS6), it is removed in SS4 and its DEATH trigger fires normally. DEATH is not an action-based hook — it fires because the unit is removed from the board, not because the unit chose to act. A SILENCE, by contrast, strips DEATH triggers for the duration.
...
**SILENCE + IRREMOVABLE:** A SILENCEd IRREMOVABLE unit loses its IRREMOVABLE keyword. It can now be displaced by REPEL, ATTRACT, TELEPORT, Spells, or Orders for the duration of the SILENCE. IRREMOVABLE's protection is keyword-based and strippable by SILENCE like all other keywords.

**SILENCE + UNTARGETABLE:** A SILENCEd UNTARGETABLE unit loses its UNTARGETABLE keyword. It can now be named as the target of opponent Spells and Orders for the duration of the SILENCE.

**SILENCE + HASTE — same-SS1 timing:** SILENCE strips the HASTE keyword. If SILENCE is applied to a HASTE unit in SS1 (via an APPEARANCE trigger before summoning-sickness removal resolves), the unit no longer has HASTE and is subject to summoning sickness for this round — it skips SS2, SS3, SS5, and SS6. This mirrors the explicit STUN+HASTE rule ("STUN applied in SS1 overrides HASTE"). If SILENCE is applied after SS1 completes, the HASTE unit has already acted in SS1 — SILENCE cannot retroactively undo movement or attacks already resolved.

**INJURED-granted keywords after SILENCE expiry:** When SILENCE expires (end of RESOLUTION R), a unit that is still INJURED at the start of RESOLUTION R+1 regains its INJURED-granted keyword bonuses — SILENCE no longer suppresses them. The INJURED state itself persists until HP is restored to max_HP or the unit dies; SILENCE only suppresses the granted bonuses while active, not the INJURED state itself.
...
**BODYGUARD no valid target at entry:** If a BODYGUARD unit enters the board in SS1 and no other friendly unit exists on the board at that moment, the bond is `None` — no protection is established. BODYGUARD still enters the board successfully. Protection can never be established retroactively; if a friendly unit enters play later in SS1, the BODYGUARD bond is NOT updated to protect it (bond is established once at the moment of BODYGUARD's SS1 entry).

**BODYGUARD CHANGE LANE (BODYGUARD unit itself):** If the BODYGUARD unit itself executes CHANGE LANE, the bond persists — the entity reference `bodyguard_protects` is stable across lane changes. The protected unit's position does not change; the connector indicator redraws from the new BODYGUARD position.

**INJURED via APPEARANCE — SS2 bonus activation:** A unit that is damaged to below max_HP by an APPEARANCE-triggered effect in SS1 becomes INJURED at the SS1→SS2 boundary (not at SS3→SS4). INJURED-granted bonuses (FIRST STRIKE, COUNTERATTACK, RANGE, SHIELD) are therefore active from SS2 onward — including SS3 (FIRST STRIKE fires in SS3) and SS6 (COUNTERATTACK, RANGE). This is a powerful synergy: card authors assigning INJURED-granted FIRST STRIKE to a unit with a self-damaging APPEARANCE trigger should evaluate the SS2-availability of the bonus carefully.
...
**TELEPORT co-occupation and subsequent combat:** TELEPORT is the only effect that can place a unit on a cell already occupied by an opposing unit (co-occupation allowed per design). After TELEPORT lands, both units participate in the remainder of RESOLUTION from the co-occupied cell. In SS5 (standard movement), each unit advances per its own MP in its own advance direction — Player A units move toward Cell 8, Player B units toward Cell 1 — and they naturally separate. If either unit has MP=0 (e.g., a WALL or a STUNned unit), the co-location persists into SS6 where they fight using standard combat resolution. For combat targeting purposes, co-located units are treated as distance=0 (within any RANGE X value); they fight in SS6 as if adjacent.
...
**LEADER grants itself its own family bonus:** A LEADER unit is counted as a family member for "X [family] in play" effects and receives its own stat bonus (if the card text grants ATK or HP to the family, the LEADER is included). The LEADER cannot "not count itself" — the snapshot includes the LEADER among the eligible recipients.

**DEATH chain with APPEARANCE spawns:** The "structural 9-link termination" note (at most 10 units on board = at most 9 deaths) assumes no new units spawn during SS4. If a DEATH trigger spawns a new unit (e.g., a class effect), the termination bound no longer holds from board count alone. The server tracks an "already-dead" set — a unit already removed cannot die again — but spawned units can themselves die and trigger DEATH chains. The RSM 60-second RESOLUTION safety timeout is the unconditional backstop.
...
| Server-side RNG | `server-rng.md` | RESOLUTION RNG chain — 3 seed slots registered in ADR-005: `RangeEquidistantSelect`, `TeleportRandomDest`, `StrichChangeLaneSelect` (OQ-KS1 RESOLVED 2026-05-01) | Hard |
...
### OUTNUMBERED design tension note

OUTNUMBERED is re-evaluated at each sub-step boundary. A player who built their board to exploit the OUTNUMBERED bonus may lose it mid-RESOLUTION if their own units inflict enough kills in SS3 (via FIRST STRIKE) to equalize board counts before SS5. This is an emergence property, not a bug — a FIRST STRIKE+CHARGE unit clearing a lane in SS3 may flip the OUTNUMBERED player from "outnumbered" to "equal" before the OUTNUMBERED bonus units act in SS5. Card authors assigning OUTNUMBERED to units with strong FIRST STRIKE or CHARGE X combinations should flag this interaction in playtest notes.
...
No interactive elements. No modal dialogs. Keywords are intended to be self-describing from their card text.

**COUNTERATTACK card text:** The simplified COUNTERATTACK rule ("fires on any non-RANGE melee attack") is derivable from card text — no first-encounter tooltip required. Card text reads: `COUNTERATTACK` with reminder text: "Retaliates when attacked by a melee unit." No special tooltip system needed. This decision resolves the R1 adjudication to add a tooltip — the tooltip is no longer required because the rule was simplified (D3, R3 design review).
...
| KW-007 | GIVEN unit X has max_HP=4, current_HP=4, and gains FIRST STRIKE when INJURED; X receives 2 damage in SS3 (reducing HP to 2), WHEN SS3 resolves, THEN X's target HP after SS3 reflects a single standard attack from X (no FIRST STRIKE bonus attack); no `FirstStrikeActivated` audit event is recorded for X in SS3. INJURED activates at the SS3→SS4 boundary — X's INJURED-granted FIRST STRIKE is available from SS4 onward (i.e., next round's SS3). | BLOCKING |
...
| KW-029c | GIVEN a Player A unit at Cell 1 is REPELled 6 cells (maximum), WHEN `repel_destination = clamp(1 + (−1)×6, 1, 8) = clamp(−5, 1, 8) = 1`, THEN the unit does not move (clamped to current cell); zero cells are traversed; no Trap triggers fire. | BLOCKING |
| KW-029d | GIVEN a Player B unit at Cell 8 is REPELled 6 cells by a Player A effect (push toward Cell 8, `advance_dir(B) = −1`, negated = +1, so push direction is toward Cell 8 for Player B), WHEN `repel_destination = clamp(8 + (+1)×6, 1, 8) = clamp(14, 1, 8) = 8`, THEN the unit does not move (clamped to current cell); zero cells are traversed. | BLOCKING |
...
| KW-033b | GIVEN Strich is in Lane 3, both Lane 2 and Lane 4 are valid, and the seeded RNG selects Lane 2, WHEN an enemy unit enters Lane 3 in SS1, THEN Strich moves to Lane 2. (Seed slot `strich_change_lane_select` registered in ADR-005 Order 6 — OQ-KS1 RESOLVED. This AC is now unblocked.) | BLOCKING |
...
| KW-034b | GIVEN unit X has COUNTERATTACK and is STUNned at the start of RESOLUTION, WHEN an enemy unit attacks X in SS6, THEN X does NOT fire COUNTERATTACK; X takes incoming damage normally; the STUN suppression of reactive hooks is confirmed. | BLOCKING |
| KW-035a | GIVEN unit A (FIRST STRIKE) kills unit B (has DEATH trigger and FINAL BLOW) in SS3, WHEN the full RESOLUTION completes, THEN the server's authoritative per-player gold total reflects the FINAL BLOW gold delta; AND unit B's removal is recorded in SS4 (not SS3) per the audit log event sequence. **Test architecture:** run SS3 and SS4 as two separate schedule invocations in the unit test; assert gold total after SS3 schedule run (before SS4 schedule run), then assert removal after SS4 schedule run. Alternatively: assert on a `GoldAuditEntry { event: FinalBlow, sub_step: 3, player_id }` entry present in the audit log after full RESOLUTION — the sub_step field is the source of truth, not inter-system read ordering. | BLOCKING |
...
| ~~KW-041~~ | ~~(removed R3 per design decision D7 — had tested the old ATTRACT "stops at caster's cell" rule, which was incorrect. D7 formalized the 1-cell-apart collision rule for enemy targets: enemy units can never occupy the same cell as an opposing unit; the ATTRACT formula was updated with branching for friendly vs enemy targets. Coverage replaced by KW-030b, KW-073, and KW-079.)~~ | ~~REMOVED~~ |
...
| ~~KW-046~~ | ~~(removed R3 per design decision D8 — had tested "LEADER placed this round grants no bonus this round," which was incorrect after D8 moved the LEADER bonus snapshot to post-SS1, making the LEADER present and the bonus live in round R)~~ | ~~REMOVED~~ |
...
| KW-051 | GIVEN a Player A unit at Cell 5 is REPELled 4 cells toward its own side (Player A advance_dir = +1, so REPEL pushes to Cell 1); Cell 3 has a lethal Trap owned by Player B; WHEN REPEL resolves, THEN the unit enters Cell 4 (1 cell traversal), Cell 3 (2nd traversal — Trap triggers on entry, deals lethal damage); unit dies at Cell 3; displacement ends at Cell 3; unit does not continue to Cell 1. | ADVISORY — promoted to BLOCKING when OQ-KS4 (Trap design) resolves |
| KW-052 | GIVEN caster at Cell 2, target at Cell 6, ATTRACT 5; Cell 4 has a lethal Trap; WHEN ATTRACT resolves, THEN the target traverses Cell 5 (no Trap), Cell 4 (Trap triggers — lethal); target dies at Cell 4; displacement ends at Cell 4; target does not continue to Cell 2. | ADVISORY — promoted to BLOCKING when OQ-KS4 (Trap design) resolves |
...
| KW-056 | GIVEN unit X has RANGE granted via INJURED; X's keyword array normally has no RANGE entry; WHEN INJURED is active at SS6 entry, THEN X attacks the nearest enemy within RANGE (use test fixture card with INJURED-granted RANGE where `max_range = 2`) without advancing; X does not trigger COUNTERATTACK from the RANGE attack (COUNTERATTACK cannot be triggered by RANGE attackers per KW-005). | BLOCKING |
...
### Missing Standalone Coverage (R3 additions)

| # | Criterion | Type |
|---|---|---|
| KW-058 | GIVEN a defender has VULNERABILITY 2 and is attacked by a unit with ATK=3 and no ARMOR-PIERCING, WHEN the modifier stack resolves at step 5, THEN ATK_effective = 3 + 2 = 5 before the AR step; the defender takes damage as if the attacker had ATK=5. | BLOCKING |
| KW-059 | GIVEN a defender has RESISTANCE 2 and is attacked by a unit with ATK=4 and no ARMOR-PIERCING, WHEN the modifier stack resolves at step 6, THEN ATK_effective = max(0, 4 − 2) = 2 before the AR step; ARMOR-PIERCING is absent so AR is not bypassed. | BLOCKING |
| KW-060 | GIVEN unit P is BODYGUARD-protected by BODYGUARD G, and G is SILENCEd, WHEN an opponent targets P with a Spell or Order, THEN P IS targetable — SILENCE has stripped G's BODYGUARD keyword, ending the protection. The Spell or Order resolves against P normally. | BLOCKING |
| KW-061 | GIVEN a unit has IRREMOVABLE and executes its own CHANGE LANE (via card text or Strich-style auto-trigger), WHEN CHANGE LANE resolves, THEN the unit's lane changes successfully — IRREMOVABLE does not prevent the unit's own movement. IRREMOVABLE only blocks externally-initiated displacement (REPEL/ATTRACT/TELEPORT/Spell/Order). | BLOCKING |
| KW-062 | GIVEN a unit has UNTARGETABLE and an opponent attempts to target it with a Spell or Order, WHEN the targeting attempt resolves, THEN the Spell or Order cannot target that unit — UNTARGETABLE blocks Spell/Order targeting. Standard combat, RANGE attacks, and AoE effects unaffected by UNTARGETABLE. | BLOCKING |

### Additional ACs (new — R3 additions)

| # | Criterion | Type |
|---|---|---|
| KW-063 | GIVEN a STUNned unit receives melee damage in SS3 or SS6, WHEN the attack resolves, THEN COUNTERATTACK does NOT fire — STUN suppresses all keyword hooks including reactive triggers. | BLOCKING |
| KW-064 | GIVEN a unit with FIRST STRIKE is co-located with an enemy WALL at SS3 entry, WHEN SS3 resolves, THEN the FIRST STRIKE unit deals damage to the WALL in SS3; the WALL deals 0 damage in response. If the FIRST STRIKE damage reduces the WALL's HP to 0, the WALL is removed in SS4. | BLOCKING |
| KW-065 | GIVEN an enemy WALL was killed by FIRST STRIKE in SS3 and removed in SS4, WHEN SS5 resolves, THEN advancing friendly units are no longer blocked at the WALL's former cell — they pass through freely. | BLOCKING |
| KW-066 | GIVEN a RANGE unit has an enemy WALL as the nearest enemy within range, WHEN SS6 (or SS3 for FIRST STRIKE) resolves, THEN the RANGE unit attacks the WALL, not any unit beyond it. | BLOCKING |
| KW-067 | GIVEN a BODYGUARD unit enters when no other friendly unit is alive on the board, WHEN SS1 resolves, THEN `bodyguard_protects = None`; the BODYGUARD fights normally this round; no error state is created. | BLOCKING |
| KW-068 | GIVEN a LEADER unit is placed in SS1 of round R (with or without HASTE), WHEN the LEADER bonus snapshot is computed at SS1-end, THEN eligible family units receive the LEADER bonus in round R — the LEADER is present at snapshot time. | BLOCKING |
| KW-069 | GIVEN a unit with SHIELD has never been attacked in round R, WHEN round R+1's RESOLUTION begins, THEN the unit's SHIELD is still active — SHIELD has no round-expiry; it persists until consumed. | BLOCKING |
| KW-070 | GIVEN unit A's DEATH trigger deals damage to unit B in SS4; unit B's DEATH trigger would deal damage to unit A (which is already dead), WHEN the DEATH chain resolves, THEN the already-dead set prevents unit A from entering the chain a second time; unit B's DEATH fires once (against any valid live targets) without causing a loop. | BLOCKING |
| KW-071 | GIVEN an IRREMOVABLE unit triggers its own CHANGE LANE movement, WHEN CHANGE LANE resolves, THEN the unit successfully moves to the adjacent lane — IRREMOVABLE does not suppress own movement. | BLOCKING |
| KW-073 | GIVEN a Player A unit with RANGE is placed with an ATTRACT X ≥ 4 targeting an enemy unit at Cell 5, caster at Cell 2, WHEN ATTRACT resolves, THEN enemy stops at Cell 3 (1 cell short of caster's Cell 2 — 1-cell-apart collision rule applied); the enemy is NOT on Cell 2. | BLOCKING |
| KW-074 | GIVEN a unit receives damage from an APPEARANCE trigger in SS1 (becoming INJURED at the SS1→SS2 boundary) and has a card-granted FIRST STRIKE when INJURED, WHEN SS3 resolves, THEN the INJURED-granted FIRST STRIKE IS active — INJURED was acquired at SS1→SS2 boundary, which is before SS3. | BLOCKING |
| KW-075 | GIVEN LEADER unit L's card grants ATK+2 to all family members, WHEN the SS1-end bonus snapshot is computed, THEN L itself also receives ATK+2 — the LEADER counts as a family member and is included in the bonus grant. Assert: L's effective ATK in SS3/SS6 = base ATK + 2. | BLOCKING |
| KW-076 | GIVEN LEADER unit L enters the board in SS1 via APPEARANCE, AND another APPEARANCE effect in the same SS1 reduces L's HP to 0 before all SS1 APPEARANCE effects complete, WHEN the SS1-end snapshot is computed, THEN L is absent from the snapshot (it was killed before snapshot time) and grants no bonus to family units in round R. | BLOCKING |
| KW-077 | GIVEN unit X is INJURED and has INJURED-granted RANGE active, WHEN SILENCE is applied to X, THEN X no longer uses RANGE targeting in SS6 — the INJURED-granted RANGE is stripped by SILENCE. X attacks as a melee unit (or not at all if no melee target is in range). Assert: X's attack in SS6 is melee (proximity-based), not RANGE (distance-based). | BLOCKING |
| KW-078 | GIVEN BODYGUARD unit G (Lane 2) protects unit P (Lane 3); G executes CHANGE LANE to Lane 1, WHEN G is in Lane 1, THEN P is still protected by G — G's `bodyguard_protects` field still references P's entity ID; an opponent Spell/Order targeting P is still blocked. The bond survived G's lane change because it is stored as a unit-to-unit entity reference, not a lane-scoped attribute. | BLOCKING |
| KW-079 | GIVEN Player A caster at Cell 2, enemy target (Player B) at Cell 3, ATTRACT 6, WHEN ATTRACT resolves, THEN `effective_pull = min(6, max(0, \|2−3\| − 1)) = min(6, max(0, 0)) = 0`; enemy stays at Cell 3 — already 1 cell apart, collision rule prevents any further pull. The `max(0, ...)` guard is load-bearing: without it, `\|dist\|−1 = 0−1 = −1` in u8 arithmetic would underflow. | BLOCKING |
| KW-080 | GIVEN unit A's DEATH trigger effect deals damage and kills unit B, WHEN B is removed in SS4 and its death is recorded, THEN kill gold (+1) for B's death is awarded to A's controller — A's DEATH trigger was the killing effect; A's controller is credited as the "attacker" for kill-gold purposes. | BLOCKING |
| KW-081 | GIVEN unit X is STUNned and has a DEATH trigger; X receives lethal damage in SS3 or SS6 and is removed in SS4, WHEN SS4 resolves, THEN X's DEATH trigger fires normally — STUN does not suppress DEATH. X's controller receives any effects specified in X's DEATH trigger definition. | BLOCKING |

### R5 Re-review Additions (R5 lean pass)

| # | Criterion | Type |
|---|---|---|
| KW-082 | GIVEN a HASTE unit is placed in SS1 and an APPEARANCE trigger in the same SS1 applies SILENCE to it before summoning-sickness removal resolves, WHEN RESOLUTION proceeds, THEN the SILENCEd unit skips SS2, SS3, SS5, and SS6 for this round — SILENCE stripped HASTE, restoring summoning sickness. Assert: no `HasteActivated` event emitted; unit is absent from SS2–SS6 participation for this RESOLUTION. | BLOCKING |

## Open Questions

| # | Question | Owner | Action Required |
|---|---|---|---|
| ~~OQ-KS1~~ | ~~Three distinct keyword RNG events need separate seed slots in the RESOLUTION chain.~~ **RESOLVED 2026-05-01.** All three seed slots registered in ADR-005 (Orders 4, 5, 6 in the RESOLUTION chain): `RangeEquidistantSelect` (SS3/SS6 equidistant RANGE), `TeleportRandomDest` (within triggering sub-step), `StrichChangeLaneSelect` (after triggering sub-step). OQ-KS3 in `combat-resolution.md` is also resolved. | — | Closed. |
...
| OQ-KS3 | COUNTERATTACK rule simplified in R3 (design decision D3): fires on any non-RANGE attack. No proximity condition. Update `combat-resolution.md` OQ4 to Resolved and update any COUNTERATTACK references to reflect simplified rule. **R6 note:** `combat-resolution.md` is now Approved — verify OQ4 was struck through and COUNTERATTACK references reflect D3 before COUNTERATTACK implementation. If confirmed, strike through this OQ. | Combat Resolution GDD | Update `combat-resolution.md` before keyword implementation. |
...
| ~~OQ-KS5~~ | ~~`combat-resolution.md` Visual section currently specifies an OUTNUMBERED indicator scoped per-lane.~~ **RESOLVED 2026-05-01.** `combat-resolution.md` Visual section updated: OUTNUMBERED indicator is now per-unit (Crimson Slate arrow-down glyph on each unit carrying the OUTNUMBERED keyword), reading from the global board-count boolean — not per-lane. | — | Closed. |
...
| ~~OQ-KS6~~ | ~~**STUN suppresses ALL reactive keyword hooks**~~ **RESOLVED R4 (design clarification).** STUN suppresses reactive keyword hooks that require the unit to actively participate (COUNTERATTACK and action-based triggers). **STUN does NOT suppress DEATH** — a STUNned unit that is killed fires its DEATH trigger normally (dying is not a unit action). D4 ruling stands for COUNTERATTACK suppression; DEATH exception added R4. | — | Closed. |
| ~~OQ-KS7~~ | ~~**SilenceApplied event payload should include stripped_keywords list.**~~ **RESOLVED R4.** `KeywordPayload::SilenceApplied` extended to `{ duration_rounds: u8, stripped_keywords: Vec<KeywordKind> }`. NP GDD must add `stripped_keywords: Vec<KeywordKind>` to the `SilenceApplied` variant before first SILENCE story. See Replication Contract SILENCE row for full spec. | — | Closed. |
| ~~OQ-KS8~~ | ~~**CounterattackFired payload must include `target_id: EntityId`.**~~ **RESOLVED R4.** `CounterattackFired` extended to `{ target_id: EntityId }`. Server emits one event per retaliation target (one per attacker in multi-attacker COUNTERATTACK). NP GDD must add `target_id: EntityId` to `CounterattackFired` variant before first COUNTERATTACK story. See Replication Contract for full spec. | — | Closed. |
| OQ-KS9 | **LEADER snapshot timing change (post-SS1) must propagate to Combat Resolution GDD.** R3 changed snapshot timing from "RESOLUTION entry" to "post-SS1." Combat Resolution GDD (if it references LEADER snapshot timing) must be updated. | Combat Resolution GDD | Update `combat-resolution.md` LEADER snapshot language before LEADER implementation story. |
| OQ-KS10 | **Board-lane-system.md must formally define the 1-cell-apart collision rule for opposing units.** The ATTRACT formula correction (D7) references this rule, but `board-lane-system.md` is the authoritative owner. The keyword system GDD references the rule but cannot define it. | Board/Lane System GDD | Add a formal rule in `board-lane-system.md` Detailed Rules section: "Two opposing units (one per player) can never occupy the same cell; when they would collide, one halts 1 cell short." Cross-reference from this GDD's ATTRACT formula. |
