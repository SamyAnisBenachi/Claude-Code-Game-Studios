...
| OQ-KS6 | **STUN suppresses ALL reactive keyword hooks** — CONFIRMED by R3 design review (D4). STUN = full shutdown including COUNTERATTACK, DEATH trigger, and all other trigger-based keywords. This ruling is binding. | Combat Resolution + Keyword System | Apply to all story ACs involving STUNned units. |
| OQ-KS7 | **SilenceApplied event payload should include stripped_keywords list.** Currently `KeywordPayload::SilenceApplied { duration_rounds: u8 }` carries only duration. The client needs to know which keywords were stripped to clear the correct glyph indicators and runtime states (e.g., INJURED-granted bonuses). Requires NP GDD update before SILENCE implementation. | Network Protocol GDD | Add `stripped_keywords: Vec<KeywordKind>` to `SilenceApplied` payload in NP GDD D.3 before first SILENCE story. |
| OQ-KS8 | **CounterattackFired payload must include `target_id: EntityId`.** In multi-attacker scenarios (KW-049), COUNTERATTACK fires against multiple attackers. The client needs `target_id` to animate each retaliation correctly. Requires NP GDD update. | Network Protocol GDD | Add `target_id: EntityId` to `CounterattackFired` payload variant before first COUNTERATTACK implementation story. |
