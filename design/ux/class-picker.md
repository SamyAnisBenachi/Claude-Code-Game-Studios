...
| **Opponent confirmed (waiting)** | `S2CSlotUpdated` received with opponent slot `class_locked: true` | No change to own picker; opponent slot in lobby header updates to "Ready ✓" — class still hidden (`S2CClassLocked` is unicast to the locking player only; opponent observes lock via `S2CSlotUpdated`) |
